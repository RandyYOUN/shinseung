<?php include("top.php");?>
        ...
        <section class="step01">
            ...
            <div>
                <h2>종합소득세 안내를 위해 아래의 정보를 입력 부탁드립니다</h2>
                <h3>입력하신 정보는 종합소득세 안내 및 신고를 위한 용도로만 사용됩니다.</h3>
                ...
                <label>홈택스 패스워드를 입력해주십시오</label><input type="password" placeholder="홈택스 패스워드" id="HTPW" name="HTPW">
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data.Status == "OK" && data.total_paid != null){
					total_paid = isEmptyNum(data.total_paid);
					
					var est_tax = CAL_AUTO(data.total_paid, data.ext_ratio, isEmptyNum(data.del_price), isEmptyNum(data.add_tax) );
