...
CASE 
	...
	ELSE IMG_URL 
    END 
) AS IMG_URL_ , CATE,ifnull((SELECT MAX(ID) FROM SS_NEWS WHERE ID < $id AND VISIBLE = 'Y'),0) AS PRE, 
ifnull((SELECT MIN(ID) FROM SS_NEWS WHERE ID > $id AND VISIBLE = 'Y'),0) AS NEXT,
ifnull(substring_index(FILE_URL, '.', -1),'down') as FILE_EXT 
FROM SS_NEWS where ID = $id AND VISIBLE='Y' ORDER BY ID DESC;") or die("SQL error");
...
?>	
	<div class="s_news">
		...
		<section class="newsview">
			...
			<?php
			if(strlen($file_url) > 0 && strlen($file_ext)>0){
			    ?>
