...
CASE 
	...
	ELSE IMG_URL 
    END 
) AS IMG_URL_, CATE,ifnull((SELECT MAX(ID) FROM SS_NEWS WHERE ID < $id AND VISIBLE = 'Y'),0) AS PRE, ifnull((SELECT MIN(ID) FROM SS_NEWS WHERE ID > $id AND VISIBLE = 'Y'),0) AS NEXT,ifnull(substring_index(FILE_URL, '.', -1),'.') as FILE_EXT FROM SS_NEWS where ID = $id AND VISIBLE='Y' ORDER BY ID DESC;") or die("SQL error");
...
?>
	<h3 style="width:250px;"><b>첨부파일 :</b><a  href="javascript:down('<?php echo $row["FILE_URL"] ?>','admin/upload/');" >&nbsp;<img width="20px" src="http://taxtok.kr/admin/resources/images/icons/<?php echo $row["FILE_EXT"] ?>.png">&nbsp;<font color="gray"><?php echo $row["FILE_URL"] ?></font></a></h3><br>
<?		
//}
?>
					<h4>
