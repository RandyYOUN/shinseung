...
$query = "SELECT count(ID) as total, round( avg(REV_SCORE),1) AS t_score,TRUNCATE( avg(REV_SCORE),0) AS t_score_star  FROM SS_REVIEW where 1=1 AND rev_cate = '종합소득세'".$STR.$STR2;
...
$QUERY = "SELECT *, phone_fn(REV_HP) AS REV_HP_, date_format(REV_REGDATE,'%Y-%m-%d') as REV_REGDATE_ FROM SS_REVIEW where rev_cate = '종합소득세' ORDER BY REV_REGDATE DESC LIMIT $limit_idx, $page_set ;";
