<?php include("top.php");?>
	...
	<div class="wrap">
		...
		<!-- section class="vat">
			<div>
				<div>
					<h1>쉽고, 싸고, 편한 부가세 신고 </h1>
					<h2>부가가치세 신고대행 수수료 안내 및<BR>부가가치세 신고시 감면 가능 여부도 함께 검토해 드립니다.</h2>
...
$QUERY = "SELECT *,LEFT(REGEXP_REPLACE(CONTENTS_, '<[^>]+>',''),100) AS CONTENTS_, date_format(NEWS_REGDATE, '%Y-%m-%d') AS REGDATE_, (CASE WHEN IFNULL(IMG_URL ,'') = '' THEN 'http://sostax.kr/news/images/default.jpg' ELSE IMG_URL END ) AS IMG_URL_    FROM dbsschina.SS_NEWS WHERE VISIBLE='Y' and cate='VAT' ORDER BY REGDATE DESC  LIMIT 0, 3 ;";
