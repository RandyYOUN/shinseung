...
{ 
	...
	{ 
		...
		$P_INF_PATH=mysqli_real_escape_string($connect,$_POST["inf_path"]);
		$output = array();
		...
		$procedure = "CREATE PROCEDURE INSERT_VAT_CST_MOBILE(IN P_CSTNAME varchar(45) CHARSET utf8, P_MOBILE varchar(45), P_INF_PATH VARCHAR(50) )
		BEGIN
			...
			IF CNT_CSTID = 0 THEN
				...
				CALL INSERT_TB100022_EX(TMP_CSTID,'부가세',DATE_FORMAT(now(), '%Y'),'1',NULL, NULL, '9',NULL,NULL,P_INF_PATH);
		...
		{ //위에서 만든 프로시저 실행
			...
			{
				$query = "CALL INSERT_VAT_CST_MOBILE('".$CSTNAME."','".$MOBILE."','".$P_INF_PATH."')";
				//프로시저 호출
	...
	{ 
		...
		BEGIN
			...
			IF IFNULL(P_EST_FEE ,'') <> '' THEN
                ...
                UPDATE TB100022 SET EST_FEE = P_EST_FEE WHERE BIZ_ID = P_BIZID;
            END IF;
