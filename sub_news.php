...
CASE 
	WHEN IFNULL(IMG_URL ,'') = '' THEN
		CASE 
			WHEN C_CATE = 'c_tra' THEN 'https://taxtok.kr/resources/images/qna_tra.jpg'
            WHEN C_CATE = 'c_inh' THEN 'https://taxtok.kr/resources/images/qna_inh.jpg'
            WHEN C_CATE = 'c_gtx' THEN 'https://taxtok.kr/resources/images/qna_give.jpg'
            WHEN C_CATE = 'c_hos' THEN 'https://taxtok.kr/resources/images/qna_hos.jpg'
			ELSE 'https://taxtok.kr/resources/images/default.png' 
        END
	ELSE 
		IMG_URL
    END 
) AS IMG_URL_ FROM dbsschina.SS_NEWS WHERE VISIBLE='Y' AND SITE_GUBUN <> 'HOS' ".$STR.$STR2." ORDER BY NEWS_REGDATE DESC  LIMIT $limit_idx, $page_set ;";
					...
					<a href="./sub_newsview.php?id=<?php echo $row["ID"]?>">
						<li><span><img src="<?PHP echo $row["IMG_URL_"]?>"></span>
							<div>
								...
								<h2><?PHP echo mb_strimwidth($fx_contents,'0','90','...','utf-8')?></h2>
...
?>
			...
			<section class="s_newsform">
				...
				<div class="form">
					...
					<button type="button" onclick="javascript:ChannelIO('show');">채팅상담 요청</button>
				</div>
