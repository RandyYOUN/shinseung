...
BEGIN
SELECT 
ID, MOBILE,CSTNAME,QUEST, MEMO,IFNULL( BRANCH,0) AS BRANCH, IFNULL(STAT,0) STAT, IFNULL(CONTACT_CNT,0) AS CNT_,date_format(REGDATE,'%Y%m%d') as REGDATE_ 
FROM INCOME_CST WHERE FLAG='INCOME' AND VISIBLE = 'Y' ".$query_str1.$query_str2.$query_str4." ORDER BY REGDATE DESC LIMIT 20;
END;
";
...
$output .= '
<tr>
<td>'.$row["ID"].'<br><button onclick="javascript:hide(\''.$row["ID"].'\');">가리기</button></td>
<td>'.$row["CSTNAME"].'</td>
<td>'.$row["MOBILE"].'</td>
<td>'.$row["QUEST"].'</td>
<td>'.$row["REGDATE_"].'</td>
