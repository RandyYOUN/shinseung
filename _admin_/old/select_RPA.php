...
<th width="3%">지점</th>
<th width="5%">이름</th>
<th width="5%">HomeTaxID</th>
<th width="7%">핸드폰</th>';
if($cst_type == "종합소득세"){
	$head .= '<th width="7%">주민등록번호</th>';
}else if($cst_type == "부가세"){
	$head .= '<th width="7%">사업자번호</th>';
}

$head .= '<th width="6%">처리시간</th>
<th width="4%">안내문</th>
<th width="5%">SmartA<br>변환</th>
<th width="5%">홈택스<br>신고</th>
<th width="8%">오류</th>
<th width="7%">소득금액(단위:원)</th>
<th width="10%">납부번호</th>
<th width="8%">지방세납부번호</th>
<th width="7%">환급은행</th>
<th width="7%">계좌</th>
</tr>
		...
		if(mysqli_query($connect,$procedure))
		{
			...
			if(mysqli_num_rows($result) >0)
			{
				...
				while($row = mysqli_fetch_array($result)){
					...
					if($cst_type == "종합소득세"){
						$output .= $row["RESIDENT_ID"].'</td><td>';
					}else if($cst_type == "부가세"){
						$output .= $row["BIZ_ID"].'</td><td>';
					}
					
					$output .= $row["REGDATE"].'</td>
					<td>'.$row["STEP1"].'</td>
					<td>'.$row["STEP3_1"].'</td>
					<td>'.$row["STEP3_2"].'</td>
					<td>'.$row["EXCEPTION"].'</td>
