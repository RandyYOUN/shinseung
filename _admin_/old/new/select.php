			...
			if(mysqli_query($connect,$procedure))
			{
				...
				if(mysqli_num_rows($result) >0)
				{
					...
					while($row = mysqli_fetch_array($result)){
						...
						<td>'.$row["USER_ID"].'</td>
						<td>
							<DIV id="lbl_'.$row["ID"].'" style="width:200px;height:25px;padding-top:8px;" onclick="javascript:switch_comp(this)">'.$row["USER_PW"].'</DIV><input type="text" id="ip_'.$row["ID"].'" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27) {select_insert(this);}" value="'.$row["USER_PW"].'" style="width:200px;height:25px;8px 10px 0px 68px; display:none;"></input></td>
						<td>
