...
if(isset($id))
{
	$procedure = "
		...
		BEGIN
			SELECT * FROM INCOME_CST WHERE ID = '".$id."';
		END;
		";
			...
			if(mysqli_query($connect,$procedure))
			{
				...
				{					
					while($row = mysqli_fetch_array($result)){
						...
						$MOBILE = $row["MOBILE"];
						$dir = $path.$CSTNAME."/";
						$down_dir = iconv("UTF-8","cp949","https://taxtok.kr/tax_income/upload/".$CSTNAME."/");
						...
						if (is_dir($dir)){                              
						  if ($dh = opendir($dir)){                     //$df = array_diff(scandir($dir),$ignore);
							while (($file = readdir($dh)) !== false){   if($file == ".." || $file == "."){
									...
									echo "<tr><td><a href='" .iconv("euc-kr","UTF-8",$down_dir). iconv("euc-kr","UTF-8",$file) . "' target=_blank>". iconv("euc-kr","UTF-8",$file) ."</a></td></tr>";        		
								}
