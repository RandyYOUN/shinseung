...
if($id != ""){
$QUERY = "SELECT *, date_format(NEWS_REGDATE, '%Y-%m-%d') AS REGDATE_ , (CASE WHEN IFNULL(IMG_URL ,'') = '' THEN 'http://sostax.kr/y2509g/news/images/tax.png' ELSE IMG_URL END ) AS IMG_URL_ , CATE,ifnull((SELECT MAX(ID) FROM SS_NEWS WHERE ID < $id AND VISIBLE = 'Y'),0) AS PRE, ifnull((SELECT MIN(ID) FROM SS_NEWS WHERE ID > $id AND VISIBLE = 'Y'),0) AS NEXT,ifnull(substring_index(FILE_URL, '.', -1),'down') as FILE_EXT FROM SS_NEWS where ID = $id AND VISIBLE='Y' ORDER BY ID DESC;";
...
?>
		<section class="s_newsform">
			...
			<div class="form">
				...
				<button type="button" onclick="javascript:window.open('http://medi-tax.kr/sub_qnawrite.php');">채팅상담 요청</button>
			</div>
