		...
		<div class="s_news">
			...
			<section class="newsnavi">
				<ul>
					...
					<a href="sub_news.php?cate=TRT" id="TRT">
						<li>세무신고</li>
					</a>
					<a href="sub_news.php?cate=TIV" id="TIV">
						<li>세무조사</li>
					</a>
...
if($CATE =="GTX" || $CATE =="INH" || $CATE =="TRA" || $CATE =="THA" || $CATE =="19T" || $CATE == "QNA" || $CATE == "ALL"){
	$STR3="";
}else{
	$STR3=" AND SITE_GUBUN = 'HOS' ";
}
/* 병의원사이트에서는 상담사례 2차뎁스 사용안함
...
if($CATE == "QNA"){
	$STR2 = " AND C_CATE = 'c_hos' ";
}

$query = "SELECT count(ID) as total FROM SS_NEWS  WHERE VISIBLE='Y' AND CATE IN ('SCH','GTX','INH','TRA','QNA','THA','19T')  ".$STR.$STR2.$STR3;
...
CASE 
	...
	ELSE IMG_URL 
    END 
) AS IMG_URL_ FROM dbsschina.SS_NEWS WHERE VISIBLE='Y' 
AND CATE IN ('SCH','GTX','INH','TRA','QNA','THA','19T') ".$STR.$STR2.$STR3." ORDER BY NEWS_REGDATE DESC  LIMIT $limit_idx, $page_set ;";
...
?>
					...
					<a href="./sub_newsview.php?id=<?php echo $row["ID"]?>">
						<li><span><img src="<?PHP echo $row["IMG_URL_"]?>"></span>
							<div>
...
?>
			...
			<section class="s_newsform">
				<img src="resources_/images/linkpeople.png">
				<div class="form">
					<h1>양도세 . 상속세 . 증여세 <br> 세무기장 . 부가세 . 종합소득세</h1>
					<button type="button" onclick="javascript:window.open('http://medi-tax.kr/sub_qnawrite.php');">상담 요청</button>
				</div>
