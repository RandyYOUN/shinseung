...
BEGIN
                ...
                ADDRESS1 = P_ADDRESS, POST_NUM = P_POSTNUM, EDTDATE=NOW(),CONFIRM='U'
                WHERE CSTID = P_CSTID;
    ...
    //사용자 배송지 확인처리 : S
    if($_POST["action"] == "upt_address_C"){
        $output = array();
        $cstid=mysqli_real_escape_string($connect,$_POST["cstid"]);
        
        $procedure = "
			CREATE  PROCEDURE `UPT_CST_ADDRESS_N`(IN P_CSTID INT(11) )
BEGIN
            
                UPDATE TB100021 SET CONFIRM='Y', EDTDATE=NOW()
                WHERE CSTID = P_CSTID;
			...
			END
			";
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS UPT_CST_ADDRESS_N"))
        {
            if(mysqli_query($connect, $procedure))
            {
                $query = "CALL UPT_CST_ADDRESS_N('".$cstid."')";
                $result = mysqli_query($connect, $query);
...
BEGIN
                ...
                UPDATE TB100021 SET CONFIRM='N', EDTDATE=NOW()
                WHERE CSTID = P_CSTID;
	...
	if($_POST["action"] == "select_cst_info_simple"){
	    ...
	    $procedure = "
			...
			BEGIN
                ...
                AND B.CST_TYPE = 'A1001' AND CST_TYPE_YEAR = DATE_FORMAT(now(), '%Y');
	...
	if($_POST["action"] == "login_cstname_mobile"){
	    ...
	    $procedure = "
			...
			BEGIN
                ...
                ON A.CSTID = B.CSTID WHERE B.CST_TYPE = 'A1001' AND B.CST_TYPE_YEAR=DATE_FORMAT(now(), '%Y')-1
                AND A.CSTNAME = P_CSTNAME 
...
BEGIN
            ...
            DECLARE CNT_TB100032 INT;
				...
				IF IFNULL(CNT,0) > 0 THEN 
                    ...
                    WHERE CSTID = TMP_CSTID AND CST_TYPE='A1001' AND CST_TYPE_YEAR=DATE_FORMAT(now(), '%Y')-1;
					
			
					IF IFNULL(TMP_BIZID,'') = '' THEN
						INSERT INTO TB100022(CSTID, REGDATE, CST_TYPE, CST_TYPE_YEAR, CST_TYPE_SEQ, INF_PATH)
						SELECT TMP_CSTID, NOW(),'A1001',DATE_FORMAT(now(), '%Y'),'1',P_INF_PATH;
