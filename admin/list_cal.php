					...
					<table>
						<tbody>
							...
							<tr>
								...
								<td>
									<label style="font-size:20px; color:red;" id="2st_payment"> </label>
			...
			if(pay < 100000000){ //1억미만
				...
				switch(gubun){
					case "IND" : st1 = Math.ceil(350000 + (tmp1*15)/10000);
								...
								break;
					case "COR" : st1 = 450000 + (tmp1*18)/10000;
								st2 = st1 + (st1 * parseFloat(add));
								query1 = "계산식 => 450,000 + ( " + inputNumberWithComma(String(tmp1))+" x 18 ÷ 10000)" ;
								query2 = "계산식 => "+inputNumberWithComma(st1)+" + ( "+inputNumberWithComma(st1)+" x " +  String(add) + " )";
								gubun_money = 450000;
								math = 18;
								break;
...
$(document).ready(function(){
	...
	$("#s_str").on("keyup", function() { // 핸드폰 숫자만입력
		 cal_test();
	});
