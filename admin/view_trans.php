				...
				<div class="boardwrite">
					<table>
						<tbody>
							...
							<TR class="tr_color" style="background:#ecff0029;height:100px;">
								<TH>진행상태</TH>
								<TD>
								
									<div class="selectbox w50">
										<label for="">선택</label>
										<select name="progress2" id ="progress2" class="w50"  onchange="javascript:modiy_progress();">
								...
								<TH>수신참조</TH>
								<TD>
									<select style="width:300px;height:35px;" id="send_add" name="send_add" onchange="javascript:send_kakao_add();">
							...
							<TR class="tr_color" style="">
								...
								<TD colspan=3>
									<span style="padding:10px; padding-top:43px; font-size:17px;" ><b><?=$username?></b></span>
									<textarea class="w50" name="comment" id="comment" style="height: 100px;"></textarea>
									<button name="reg_comment" id="reg_comment" style="width:80px;border: 1px solid #444;background: #fff;height: 100px;padding: 10px 15px;letter-spacing: 0.5px;margin: 0 3px 0;" >등록</button>
								</TD>
							</TR>
							<TR class="tr_color" style="">
...
function modiy_progress(){
	var request = new Request();
	var id = request.getParameter("id");
	var action = "action_trans_prog_upt";
	var prog = $("select[name=progress2]").val();
	//var add_member = $("#send_add").val();
	...
	if(id != ""){
		$.ajax({
			url:"action.php",
			method:"POST",
			data:{id:id,action:action, prog:prog},
			success:function(data){
				//리스트 다시 조회
				if(data=="success")
					fetchUser();
...
function send_kakao_add(){
	var request = new Request();
	var id = request.getParameter("id");
	var userid = "<?= $userid ?>";
	var add_member = $("#send_add").val();
	var action = "action_trans_send_prog_add_member";
...
function send_kakao(str){
	...
	var userid = "<?= $userid ?>";
	var comment = str;
	var action = "action_trans_send_comment";
	...
	if(comment != ""){
		...
		//insert page로 위에서 받은 데이터를 넣어준다.
			url:"api/send_tok_v1.php", 
			method:"POST",
			data:{id:id,userid:userid,action:action,comment:comment},
			success:function(data){
				console.log(data);
			}
		});
