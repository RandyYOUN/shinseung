...
{ //db연결
	...
	if($_GET["action"] == "action_send_tok_complate"){
	    ...
	    $procedure = "
		CREATE PROCEDURE SELECT_SEND_TOK_COMPLATE()
		BEGIN
			SELECT A.CSTNAME ,
	    ...
	    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_SEND_TOK_COMPLATE"))
	    {
	        ...
	        {
	            $query = "CALL SELECT_SEND_TOK_COMPLATE()";
...
감사합니다.";
	                    send_kakao_all($CSTNAME, $MOBILE, $INCOME_TAX,$REPORT_NUM_INCOME,$JIBANG_TAX,$REPORT_NUM_WETAX,$BRANCH,$BIZ_ID ,$message);
...
function send_kakao_all($CSTNAME, $MOBILE, $INCOME_TAX,$REPORT_NUM_INCOME,$JIBANG_TAX,$REPORT_NUM_WETAX,$BRANCH,$BIZ_ID ,$message){
    ...
    $at = array(
        ...
        "templatecode" => "bizp_2021042718185226670364665",
        "message" => $message,
        "button" => $button
    );
    ...
    if($ck_cnt == 0){
        ...
        // 알림톡 발송
        /*DB로그 남기기*/
        send_kakao_log($BIZ_ID, $MOBILE, $template_id, $sender_id,'AUTO_COMPLATE',$flag, $flag2, $userid);
    }else{
        echo 'error:abuse';
    }
}
