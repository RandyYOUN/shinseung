...
[data-tooltip-text]:hover:after {
  ...
  margin-bottom: 10px;
	...
	z-index: 9999;
		...
		<div class="content" style="width:1822px;">
			<div class="navi">
			</div>
			...
			<div class="conwrap">
				...
				<div class="search">
    					<div class="selectbox w150p" >
    						<label for="">채널선택</label>
    						<select name="inf_flag" id ="inf_flag">
    							<option value="">유입채널-전체</option>
    							<option value="cktk">채널톡</option>
    							<option value="iq200">iq200</option>
    						</select>
    					</div>
...
?>
	...
	<input type=hidden id="page_flag" value="부가세 영업현황">
...
$('#b_option').on('change',function(){
	var req = new Request();
	var inf_flag = req.getParameter("infflag");
	
	var b_option = $('#b_option').val();
	var newURL = window.location.protocol + "//" + window.location.host + window.location.pathname;
	window.location.href=newURL  + "?g_option="+$('#g_option').val()+"&b_option="+escape($('#b_option').val())+"&s_str="+escape($('#s_str').val())+"&infflag="+inf_flag;
});



$('#inf_flag').on('change',function(){

	var inf_flag = $('#inf_flag').val();
...
$(document).ready(function(){
	...
	function fetchUser()
	{
		...
		var action = "select_list_reginfo_VAT";
		var b_option = $('#b_option').val();
		var g_option = $('#g_option').val();
		var s_str = $('#s_str').val();
		var cst_type = "A1002";//부가세
		var req = new Request();
		var page = req.getParameter("page");
		var inf_flag = req.getParameter("infflag");
		...
		$.ajax({
			url:"select.php",
			method:"POST",
			data:{action:action,s_str:s_str,b_option:b_option,g_option:g_option, page:page,cst_type:cst_type, inf_flag:inf_flag},
	...
	$('#excel').click(
			function() {
				...
				var userid="<?= $userid ?>";
				var depid="<?= $depid ?>";
				var go_url = "excel_down_acc.php?s_option="+s_option+"&g_option="+g_option+"&s_str="+s_str+"&depth="+depth+"&userid="+userid+"&depid="+depid+"&s_date1="+s_date1+"&s_date2="+s_date2;
...
function switch_name(obj){ // 발송용 이름 클릭시 입력창 노출함수
	var id_tmp = $(obj).attr("id");
	var id = id_tmp.replace("name_lbl_","");

	document.getElementById("name_lbl_"+id).style.display = "none";
	document.getElementById("name_ip_"+id).style.display = "block";

	document.getElementById("name_ip_"+id).focus();
}


function name_submit(obj){ // 발송용 이름 저장

	var id_tmp = $(obj).attr("id");
	var id = id_tmp.replace("name_ip_","");
	var name_id = "name_ip_"+id;

	var name = document.getElementById(name_id).value;
	var action = "upt_name_kakao_vat";
		...
		if(flag=="self_1"){
			...
			if(yn){
				...
				//insert page로 위에서 받은 데이터를 넣어준다.
					...
					method:"POST",
					data:{cstid:cstid,bizid:bizid, action:action,tmp_flag:tmp_flag,userid:userid, flag_vat:flag_vat},
					//dataType:"json",
					success:function(data)
					{
						console.log(data);
...
function select_fn(){
	...
	if (inf_flag != "") {
		$('#inf_flag').val(inf_flag).prop("selected",true);
		var select = $('#inf_flag');
