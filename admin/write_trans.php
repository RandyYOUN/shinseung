							...
							<tr>
								...
								<td>
									<div class="selectbox w50">
										...
										<option value="E5009">검토 요청</option>
										<option value="E5010">검토 완료</option>
...
*/
			if(data.FILE_REAL_STR){
				...
				var file_dir = "../FILE_SVR_1/trans/"+data.CSTNAME+"_"+data.MOBILE.replace(/-/gi, "")+"/";
...
//			$("#option_price").attr('style','display:block');
	...
	$('#cstname').click(function(){
		//alert('이름을 수정할 경우 이전에 업로드했던 첨부파일이 다운로드 되지 않을 수 있습니다. \n그래도 수정하시겠습니까?');

		var cstname =$("#cstname").val();
		var flag = $('#action1').text();
		
		if(cstname != "" && flag == "수정"){
			if( confirm('이름을 수정할 경우 이전에 업로드했던 첨부파일이 다운로드 되지 않을 수 있습니다. \n그래도 수정하시겠습니까?') ){
				$("#cstname").attr("Readonly",false);
				$("#cstname").focus();
			}
		} 
...
function modify_(){
		...
		if(total_tax >= limit_tax && contents.indexOf("신승의견") > -1){
			send_kakao_up5k('H1004'); // 5천만원 이상시 카톡발송
		}
		...
		if(cstname !="" && mobile !=""){
			...
			//insert page로 위에서 받은 데이터를 넣어준다.
				url:"action.php", 
				method:"POST",
				data:{id:id,progress:progress,reguser:reguser,reg_branch:reg_branch,tax_flag:tax_flag,cstname:cstname, 
					mobile:mobile,cst_address:cst_address,trans_target:trans_target,pay_flag:pay_flag,
					pay_date:pay_date,price:price,price2:price2,trans_date:trans_date,acq_date:acq_date,delivery_flag:delivery_flag,
					trans_price:trans_price,acq_price:acq_price,deadline:deadline,total_tax:total_tax,contents:contents, 
					action:action, file_real_str:file_real_str, file_view_str:file_view_str, cate:cate, owner:owner, num:num, 
					prio_num:prio_num , rep_num:rep_num,rep_date:rep_date, reg_date:reg_date,option_price:option_price,etc:etc,
					userid:userid},
				success:function(data){
					alert(data);
					if(id != ""){
...
/*컨설팅 카카오 발송*/
function send_kakao_up5k(str){

	var request = new Request();
	var cstid = request.getParameter("id");
	//var cstid = str.replace("vip_","");
	var cons = str;
	var action = "action_tok_trans_up5k";
	...
	if(cons!=""){
		...
		//insert page로 위에서 받은 데이터를 넣어준다.
			...
			method:"POST",
			data:{cstid:cstid,cons:cons,action:action,userid:userid,contents_flag:contents_flag},
			success:function(data){
				console.log(data);
