...
if($action == "dev"){
	...
	$output = array();
	$procedure = "
		CREATE PROCEDURE SELECT_NEW_NOTIFY(IN user_id int(100))
		BEGIN
			SELECT * FROM dbsschina.TB500020 WHERE NOTIFY=0 AND USERID = userid;
		END;
		";
	
		if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_NEW_NOTIFY"))
		{
			if(mysqli_query($connect, $procedure))
			{
				$query = "CALL SELECT_NEW_NOTIFY('".$_POST["id"]."')";
		...
		if(isset($tmp_id)){
			$procedure_upt = "
				CREATE PROCEDURE UPT_NOTIFY(IN tmp_id int(100))
				BEGIN
					UPDATE TB500020 SET NOTIFY=1 WHERE ID = tmp_id ;
				END;		
			";
			if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS UPT_NOTIFY"))
			{
				if(mysqli_query($connect,$procedure_upt))
				{
					$query_upt = "CALL UPT_NOTIFY('".$tmp_id."')";
                ...
                if($result)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
                            ...
                            <td>
							<select id="proc_'.$row["BIZ_ID"].'" name="proc_'.$row["BIZ_ID"].'">';
...
// 메인 > 멤버리스트 모바일 : 시작
if ($action == "select_member_main_m") {
	...
	if($d_option != ""){
		switch($d_option){
			case "username" : 
				$query_str .= " AND A.USERNAME like '%".$s_str2."%' ";
				break;
			case "depname" : 
				$query_str .= " AND B.VALUE_ like '%".$s_str2."%' ";
				break;
			case "phone" : 
				$query_str .= " AND (A.MOBILE LIKE '%".$s_str2."%' OR A.OUTER_PHONE LIKE '%".$s_str2."%') ";
				break;
			default:
				$query_str =" ";
		}	
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    $output .= '
            					<tr>
									...
									<td style="text-align: left;padding:15px 10px;">'.$row['USERNAME'];
									
									/* 직급없는사람 괄호 처리*/
										$row['POSITION'] != '' ? $output .= '('.$row['POSITION'].')' : $output .='';
									/**/
					$output .='		</td>
									<td><a href="tel:'.$row['OUTER_PHONE'].'"  class="telCompany">회사</a><a class="telPhone" href="tel:'.$row['MOBILE'].'">핸드폰</a></td>
								</tr>
            					';
                }
...
if ($action == "select_member_main") {
    ...
    $procedure = "
            	...
            	BEGIN
					...
					LEFT OUTER JOIN TB750010 AS B ON A.DEPID = B.CODE_
					LEFT OUTER JOIN TB750010 AS C ON A.POSITION_ID = C.CODE_
					WHERE DEPID NOT IN ('D1000','D1999')
					AND USERID NOT IN(1167,1168,1169,1171,1175);
...
if ($action == "select_link") {
	...
	if($s_option != ""){
		$WHERE_STR .= " AND A.CODE_ = '".$s_option."' AND LINK_NAME LIKE '%".$s_str1."%'  ";
	}else{
		$WHERE_STR .= " ";
	}
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    $output .= '
            					<li>
                                    ...
                                    <a  href="'.$row['LINK'].'">'.$row['LINK_NAME'].'</a>
                                </li>
            					';
                }
		...
		if(mysqli_query($connect,$procedure))
		{
			...
			if(mysqli_num_rows($result) >0)
			{
				...
				while($row = mysqli_fetch_array($result)){
					...
					$output .= '
					<tr >
						<td style="text-align:center;padding:0px;">'.$row["NUM"].'</td>
						<td ><a href="view_4insu.php?id='.$row["ID"].'" >'.$row["SUBJECT"].'</a></TD>
						<td >'.$row["QUEST_FLAG_"].'</TD>
						...
						<td >'.$row["REGDATE_"].'</TD>
						<td >
							<select style="height:35px;" id="'.$row["ID"].'" name="PROGRESS" onchange="javascript:mod_prog(this);">';
						...
						$prog == "E7003" ? $output .= '<option value="E7003" selected>최종완료</option>' : $output .='<option value="E7003">완료</option>';
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
					...
					<tr >
						...
						<td style="text-align:center;padding:0px;">'.$row["PRIO_NUM_"].'</td>
						<td >'.$row["PROGRESS_"].'</td>
						<td><a class="trans_link" href="view_trans.php?id='.$row["ID"].'" > '.$row['CSTNAME'].'</a></td>
						<td>'.$row["MOBILE"].'</td>
						<td>'.$row["TAX_FLAG"].'</td>
						...
						<td>'.$row["REGDATE_"].'</td>
						<td style="text-align:center;padding:0px;">'.$row["OWNER_"].'</td>
						    
						<td>'.$row["DEADLINE_"].'</td>
						    
						<td>'.$row["PRICE_"].'</td>
						<td style="text-align:center;padding:0px;">'.$row["PAY_FLAG"].'</td>
						<td>'.$row["ETC"].'</td>
...
if($action == "select_money_list"){
	...
	$query_cnt = "SELECT count(ID) as total FROM dbsschina.TB750020_Y 
			WHERE 1=1 ".$WHERE_STR;
	...
	if($_POST["s_option"] != ""){
		switch($s_option){
			case "cstname" : 
				$query_str1 .= " AND CODE_TO_USERNAME(INSERT_USER) like '%".$s_str."%' ";
				break;
			case "value" : 
				$query_str1 .= " AND VALUE_ like '%".$s_str."%' ";
				break;
			case "num" : 
				$query_str1 .= " AND ID = '".$s_str."' ";
				break;
			default:
				$query_str1 ="";
		}	
	...
	$procedure = "
	CREATE PROCEDURE SELECT_MONEY_LIST()
	BEGIN
		SELECT ID, CODE_TO_STR_MONEY(DEPT) DEPT_,DEPT,CODE_TO_STR_TYPE2(CODE_) CODE_2,CODE_, VALUE_, 
        CODE_TO_USERNAME(INSERT_USER) INSERT_USER_,INSERT_USER, MONEY,format(MONEY,0) MONEY_, 
        CASE WHEN PAY_FLAG='card' THEN '카드'
            WHEN PAY_FLAG='cash' THEN '현금'
            END as 'PAY_FLAG_' , PAY_FLAG,
        date_format(REGDATE,'%Y-%m-%d %H:%i') REGDATE_
	...
	if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_MONEY_LIST"))
	{
		if(mysqli_query($connect,$procedure))
		{
			$query = "CALL SELECT_MONEY_LIST()";
			$result = mysqli_query($connect,$query) or die(mysqli_error($connect));
			...
			if(mysqli_num_rows($result) >0)
			{
				...
				while($row = mysqli_fetch_array($result)){
				    ...
				    $dept = $row["DEPT"];
				    $code = $row["CODE_"];
				    $user = $row["INSERT_USER"];
				    $card = $row["PAY_FLAG"];
					$output .= '
					<tr >
						<td><a style="padding:0;" href="javascript:del('.$row["ID"].')">'.$row["ID"].'</a></td>
						<td>

                            <DIV id="dept_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['DEPT_'],'0','40','...','utf-8').'</DIV>
                            <select style="width:65px; height:40px;display:none;" id="dept_lbl_'.$row["ID"].'_ip" name="dept_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" onfocusout="javascript:switch_comp_rev(this);">';
                    
				               $dept == "type1" ? $output .= '<option value="type1" selected>고정</option>' : $output .= '<option value="type1">고정</option>';
				               $dept == "type2" ? $output .= '<option value="type2" selected>변동</option>' : $output .= '<option value="type2">변동</option>';
                                
                 $output .='</select>                           
                        </TD>
						<td>
                            <DIV id="code_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['CODE_2'],'0','40','...','utf-8').'</DIV>';
                            
                                if($dept == "type2" && $code =="" ){
                                    $output .='<select style="width:90%; height:40px;" id="code_lbl_'.$row["ID"].'_ip" name="code_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" >';
                                    $output .= '<option value="" selected></option>';
                                    $output .= '<option value="A1015">식비</option>';
                                    $output .= '<option value="A1016">외식비</option>';
                                    $output .= '<option value="A1017">생필품비</option>';
                                
                                }else{
                                    if($code !=""){
                                        $output .='<select style="width:86px; height:40px;display:none;" id="code_lbl_'.$row["ID"].'_ip" name="code_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" onfocusout="javascript:switch_comp_rev(this);" >';
                                        $code == "A1015" ? $output .= '<option value="A1015" selected>식비</option>' : $output .= '<option value="A1015">식비</option>';
                                        $code == "A1016" ? $output .= '<option value="A1016" selected>외식비</option>' : $output .= '<option value="A1016">외식비</option>';
                                        $code == "A1017" ? $output .= '<option value="A1016" selected>생필품비</option>' : $output .= '<option value="A1017">생필품비</option>';
                                    }
                 ...
                 $output .='</select>                           
						...
						<td>
                            <DIV id="user_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['INSERT_USER_'],'0','40','...','utf-8').'</DIV>
                            <select style="width:65px; height:40px;display:none;" id="user_lbl_'.$row["ID"].'_ip" name="user_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" onfocusout="javascript:switch_comp_rev(this);">';
                    
				               $user == "yhd" ? $output .= '<option value="yhd" selected>형덕</option>' : $output .= '<option value="yhd">형덕</option>';
				               $user == "ljh" ? $output .= '<option value="ljh" selected>지혜</option>' : $output .= '<option value="ljh">지혜</option>';                                
                 $output .='</select>                           
                        </TD>
						<td>';
                 ...
                 if($card != ""){
                     $output .='<DIV id="pay_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['PAY_FLAG_'],'0','40','...','utf-8').'</DIV>
                            <select style="width:90%; height:40px;display:none;" id="pay_lbl_'.$row["ID"].'_ip" name="pay_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" onfocusout="javascript:switch_comp_rev(this);">';
                 }else{
                     $output .='
                            <select style="width:90%; height:40px;" id="pay_lbl_'.$row["ID"].'_ip" name="pay_lbl_'.$row["ID"].'_ip" onchange="javascript:modify_option(this);" onfocusout="javascript:switch_comp_rev(this);">
                            <option value="" selected></option>    
                            ';
                                
                     
                 }
                             $card == "card" ? $output .= '<option value="card" selected>카드</option>' : $output .= '<option value="card">카드</option>';
                             $card == "cash" ? $output .= '<option value="cash" selected>현금</option>' : $output .= '<option value="cash">현금</option>';
                 $output .='</select>                           
                        </TD>
						<td><DIV id="date_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['REGDATE_'],'0','40','...','utf-8').'</DIV><input style="width:80px; height:25px;display:none;" type="date" id="date_lbl_'.$row["ID"].'_ip" value="'.$row['VALUE_'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)money_submit(this);" onfocusout="money_submit(this);switch_comp_rev(this);"  ></input></td>
						<td><DIV id="value_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['VALUE_'],'0','40','...','utf-8').'</DIV><input style="width:80px; height:25px;display:none;" type="text" id="value_lbl_'.$row["ID"].'_ip" value="'.$row['VALUE_'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)money_submit(this);" onfocusout="money_submit(this);switch_comp_rev(this);"  ></input></td>
						<td><DIV id="money_lbl_'.$row["ID"].'" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['MONEY_'],'0','40','...','utf-8').'</DIV><input style="width:80px; height:25px;display:none;" type="text" id="money_lbl_'.$row["ID"].'_ip" value="'.$row['MONEY'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)money_submit(this);" onfocusout="money_submit(this);switch_comp_rev(this);"></input></td>
					</tr>
					';
				}
			}
...
//주간통계 list select : 시작
if($action == "select_week_total"){
        $query = "SELECT TOTAL_WEEK_MONEY() AS 'TOTAL_WEEK',  TOTAL_MONTH_MONEY() AS 'TOTAL_MONTH';";
        $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
        ...
        if(mysqli_num_rows($result) >0)
        {
            ...
            while($row = mysqli_fetch_array($result)){
                ...
                $output .= '
				<tr >
                    <td>'.$row["TOTAL_MONTH"].'원</td>					
                    <td>'.$row["TOTAL_WEEK"].'원</td>
				</tr>
				';
            }
        }
...
if($action == "select_new_m"){
	...
	if($s_str !=""){
		switch($s_option){
			case "subject" : 
				$query_str1 .= " AND SUBJECT like '%".$s_str."%' ";
				break;
			case "news_reguser" : 
				$query_str1 .= " AND NEWS_REGUSER like '%".$s_str."%' ";
				break;
			case "id" : 
				$query_str1 .= " AND ID = '".$s_str."' ";
				break;
			case "contents" : 
				$query_str1 .= " AND CONTENTS_ like '%".$s_str."%' ";
				break;
			default:
				$query_str1 .="  ";
			}	
	...
	if(isset($_POST["action"]))
	{
		...
		CREATE PROCEDURE selectNews()
		BEGIN
		SELECT * FROM SS_NEWS WHERE 1=1 ".$query_str1.$query_desc."
		LIMIT $limit_idx, $page_set ;
		END;
		";
		...
		if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS selectNews"))
		{
			if(mysqli_query($connect,$procedure))
			{
				...
				if(mysqli_num_rows($result) >0)
				{
					...
					<tr>
					<td style="text-align:center">'.$row["ID"].'</td>
					<td style="text-align:left;"><a href="/admin_newsview.php?id='.$row["ID"].'" target=_blank class="news_link" >'.$row["SUBJECT"].'</a></td>
					<td>'.$row["NEWS_REGUSER"].'</td>
					<td>'.$row["NEWS_REGDATE"].'</td>
					</tr>
					';
