...
{ //db연결
	...
	{ //ajax로 넘긴 data를 받아준다.
		...
		$code=mysqli_real_escape_string($connect,$_POST["tag_code"]);
	...
	*/
	//insert 프로시저 생성
		$procedure = "
		CREATE PROCEDURE insertTag(IN SS_TAG varchar(45),PARENT_ID INT , CODE VARCHAR(20) )
		BEGIN
		INSERT INTO SS_TAG(SS_TAG,PARENT_TABLE, PARENT_ID, CODE) 
		VALUES(SS_TAG,'SS_NEWS',PARENT_ID, CODE);
		END
		";
	...
	//기존에 프로시저가 있으면 삭제
		...
		{ //위에서 만든 프로시저 실행
			...
			{
				$query = "CALL insertTag('".$tag ."',".$id.",'".$code."')";
				//프로시저 호출
				mysqli_query($connect,$query);
				echo '성공적으로 입력 되었습니다.';
			}else{
				echo 'error : ' .$tag .",".$id.",".$code ;
			}
		}else{
			echo 'error : ' .$tag .",".$id.",".$code ;
		}
