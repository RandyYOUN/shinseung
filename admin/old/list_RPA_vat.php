...
<TABLE class="table table-bordered" style="width:1640px;">
	<TR><TD COLSPAN=5><b>기본정보 스크래핑</b></TD></TR>
	<TR>
	...
	<TR><TD COLSPAN=5><b>안내문 출력</b></TD></TR>
	<TR>
		...
		<TD WIDTH="25%"><SPAN STYLE="COLOR:RED;">※</SPAN>&nbsp;사업자번호  
		<input type="text" name="BIZ_ID1" id="BIZ_ID1" class="" STYLE="PADDING:6px 12px;width:60px;" />&nbsp;-&nbsp;
		<input type="text" name="BIZ_ID2" id="BIZ_ID2" class="" STYLE="PADDING:6px 12px;width:50px;" />
		&nbsp;-&nbsp;
		<input type="text" name="BIZ_ID3" id="BIZ_ID3" class="" STYLE="PADDING:6px 12px;width:80px;" />
		</TD>
		<TD COLSPAN="2"></TD>
		
	</TR>
	<TR><TD COLSPAN=5><b>Smart A 제작, 전자신고 ,접수증&납부서 PDF다운</b></TD></TR>
	<TR>
...
$query = "SELECT count(CSTID) as total FROM TB100020 where CST_TYPE = '부가세'    ".$query_str1.$query_str2;


$QUERY_STR = "&s_option=".$_GET["s_option"]."&g_option=".$_GET["g_option"]."&s_str=".$_GET["s_str"]."&sort=".$_GET["sort"]."&flag=".$_GET["flag"]."&b_option=".$_GET["b_option"];
...
$(document).ready(function(){
	...
	$('#action').click(function(){
		...
		var cst_type = "부가세";
		var biz_id = $('#BIZ_ID1').val()+$('#BIZ_ID2').val()+$('#BIZ_ID3').val();
		...
		if( (branch!=''&& resident1 != ''&& resident2 != '') || (cstname != '') || (hometaxid !='' && hometaxpw !='' )) {
			...
			//insert page로 위에서 받은 데이터를 넣어준다.
				url:"action.php", 
				method:"POST",
				data:{numbering:numbering, cstname:cstname, mobile:mobile, resident1:resident1,resident2:resident2,ref_bank:ref_bank,ref_acc:ref_acc ,action:action, branch:branch, server:server, server_num:server_num,hometaxid:hometaxid,hometaxpw:hometaxpw ,cst_type:cst_type, biz_id:biz_id  },
				success:function(data){
					alert(data);
					location.reload();
				}
