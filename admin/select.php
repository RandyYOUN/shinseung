...
if($action == "dev"){
    ...
    $e_option4 = $_POST["e_option4"];
    $rpa_test = $_POST["rpa_test"];
    ...
    if($rpa_test == 'Y'){
        $query_desc = " ORDER BY RPA_TEST_TYPE, B.EDTDATE DESC, B.BIZ_ID DESC";
    }else{
    ...
    AND B.CST_TYPE = 'A1001' ".$query_str1.$query_str2.$query_str3.$query_str4.$query_str_test;
...
<th>구분</th>
            ...
            <th>결재</th>
<th>장부</th>
            <th colspan=2>스마트A<br>신고서작성</th>
            <th colspan=2>위하고T<br>신고서작성</th>			
<th>접수메모</th>
    ...
    if(isset($_POST["action"]))
    {
        ...
        $procedure = "
	CREATE PROCEDURE SELECT_LIST_ACC_DEV()
	BEGIN
	SELECT
    A.CSTID AS 'ID',
    B.BIZ_ID,
...
STEP_NAME( F.WehagoBookMake ) AS WEHAGO_BOOK,
    ...
    B.RP_SEND_KAKAO,
RPA_TEST_TYPE(RPA_TEST_TYPE) AS 'RPA_TEST_TYPE'
	FROM TB100020 AS A
	LEFT OUTER JOIN TB100022 AS B ON A.CSTID=B.CSTID
	LEFT OUTER JOIN TB100030 AS C ON A.CSTID=C.CSTID  AND B.BIZ_ID = C.BIZ_ID
	LEFT OUTER JOIN TB980010 AS D ON D.USERID = B.DEC_REGUSER
    LEFT OUTER JOIN TB980010 AS D2 ON D2.USERID = B.REGUSER
    LEFT OUTER JOIN TB100026 AS E ON B.BIZ_ID = E.BIZ_ID AND E.PROGRESS IN ('E7208','E7210','E7211','E7212','E7209','E7214','E7213','E7215')
    LEFT OUTER JOIN TB100023 AS F ON B.BIZ_ID = F.BIZ_ID
	WHERE 1=1
    AND IFNULL(E.PROGRESS ,'') <> ''
    AND B.CST_TYPE = '".$cst_type."' ".$query_str1.$query_str2.$query_str3.$query_str4.$query_str_test.$query_desc.
    " LIMIT $limit_idx, $page_set
	;
	END;
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_LIST_ACC_DEV"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_LIST_ACC_DEV()";
                ...
                if($result)
                {
                    ...
                    $path = "../tax_income/upload/$SET_YEAR/";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
                            ...
                            $consign=$row["CONSIGN"];
                            $confirm=$row["CONFIRM"];
                            $smarta_book=$row["SMARTA_BOOK"];
                            $wehago_book=$row["WEHAGO_BOOK"];
                            ...
                            //$down_dir = "../tax_income/upload/".$row["CSTNAME"]."_"select_view_ext.$row["MOBILE"]."/";
                            ...
                            if($smarta_book=='에러')$smarta_book_style="style='background-color:#d10000;color:white;'";
                            if($smarta_book=='예약')$smarta_book_style="style='background-color:#007cc5;color:white;'";
                            if($smarta_book=='완료')$smarta_book_style="style='background-color:green;color:white;'";
                            if($smarta_book=='실행중')$smarta_book_style="style='background-color:#c57900;color:white;'";
                            
                            if($wehago_book=='에러')$wehago_book_style="style='background-color:#d10000;color:white;'";
                            if($wehago_book=='예약')$wehago_book_style="style='background-color:#007cc5;color:white;'";
                            if($wehago_book=='완료')$wehago_book_style="style='background-color:green;color:white;'";
                            if($wehago_book=='실행중')$wehago_book_style="style='background-color:#c57900;color:white;'";
                            ...
                            if($rpa_test=='Y'){
                                $output .='<td>'.$row["RPA_TEST_TYPE"].'</td>
';
                            }
                            
                            $output .='<TD><a href="javascript:fn_mod('.$row["ID"].');">수정</a></TD>
							<td>'.$row["MOBILE"].'</td>
...
<td>'.$douzone_code.'</td>';
                            ...
                            $output .= '</td>';
                            $output .= '<td><a target=_blank href="view_acc_cal.php?id='.$row["ID"].'">링크</a></td>';
                            
                            if($smarta_book != ''){
                                $output .= '<td>';
                                if($smarta_book=="에러"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_A\','.$row["ID"].');">요청</a></td>
                                    <td '.$smarta_book_style.'><span data-tooltip-text="'.$row["ERROR_MSG"].'" >'.$smarta_book.'</span></td>';
                                }elseif($smarta_book=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_A\','.$row["ID"].');">재요청</a></td>
                                    <td '.$smarta_book_style.'>'.$smarta_book.'</td>';
                                }else{
                                    $output .= '</td><td '.$smarta_book_style.'>'.$smarta_book.'</span></td>';
                                }
                            }else{
                                $output .= '<td><a href="javascript:cal_RPA(\'step_A\','.$row["ID"].');">요청</a></td>
                                <td></td>';
                            }
                            ...
                            if($wehago_book != ''){
                                $output .= '<td>';
                                if($wehago_book=="에러"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_T\','.$row["ID"].');">요청</a></td>
                                    <td '.$wehago_book_style.'><span data-tooltip-text="'.$row["ERROR_MSG"].'" >'.$wehago_book.'</span></td>';
                                }elseif($wehago_book=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_T\','.$row["ID"].');">재요청</a></td>
                                    <td '.$wehago_book_style.'>'.$wehago_book.'</td>';
                                }else{
                                    $output .= '</td><td '.$wehago_book_style.'>'.$wehago_book.'</span></td>';
                                }
                            }else{
                                $output .= '<td><a href="javascript:cal_RPA(\'step_T\','.$row["ID"].');">요청</a></td>
                                <td></td>';
                            }
                            
                            $output .= '<TD><DIV id="memo_lbl_'.$row["ID"].'" style="width:200px;height:28px;padding-top:8px;" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['MEMO'],'0','40','...','utf-8').'</DIV><input style="width:200px; height:28px;display:none;" type="text" id="memo_ip_'.$row["ID"].'" value="'.$row['MEMO'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)memo_submit(this);" ></input></TD>
                                        ';
...
/*종합소득세 전문상담 select : 시작*/
if($action == "select_list_pro"){
    
    //ajax로 넘긴 데이터 값은 "select"
    //값이 존재하면 true를 리턴
    $cst_type = $_POST["cst_type"];
    $prog_option = $_POST["prog_option"];
    $g_option = $_POST["g_option"];
    $b_option = $_POST["b_option"];
    $s_str = $_POST["s_str"];
    $inf_channel = $_POST["inf_channel"];
    ...
    $query = "SELECT count(1) as total FROM TB100020 AS A
	LEFT OUTER JOIN TB100022 AS B ON A.CSTID=B.CSTID
    LEFT OUTER JOIN TB100026 AS F ON F.BIZ_ID = B.BIZ_ID	
    LEFT OUTER JOIN TB100030 AS C ON A.CSTID=C.CSTID
	LEFT OUTER JOIN TB980010 AS D ON D.USERID = B.DEC_REGUSER
	LEFT OUTER JOIN TB300031 AS E ON A.CSTID=E.CSTID
    WHERE B.CST_TYPE = 'A1001' AND B.CST_TYPE_YEAR = '$SET_YEAR' 
    AND B.INF_PATH='전문상담' ".$query_str1.$query_str2.$query_str3.$query_str_prog.$query_str_my;
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_PRO_COUN()
	BEGIN
	SELECT
    A.CSTID AS 'ID',
    CODE_TO_STR(B.REG_BRANCH) AS 'REG_BRANCH_PATH',
...
substring( REPLACE(A.MOBILE,'-',''),4) MOBILE_PATH,
    ...
    AND CST_TYPE_YEAR = $SET_YEAR ".$query_str1.$query_str2.$query_str3.$query_str_prog.$query_str_my.$query_desc.
	" LIMIT $limit_idx, $page_set
	;
	END;
	";
        ...
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_PRO_COUN"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_PRO_COUN()";
                ...
                if(mysqli_num_rows( $result) > 0)
                {
                    ...
                    $cate_name="";
                    $path_pay = "../tax_income/upload/$SET_YEAR/";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
                            ...
                            $kakao_1 = $row["KAKAO_SEND_1"];
                            $kakao_2 = $row["KAKAO_SEND_2"];
                            $exp_pay = $row["EXP_PAY_TAX_SELF"];
                            $est_fee_self = $row["EST_FEE_SELF"];
                            $cnt_kakao = $row["CNT_KAKAO_PROLIST"];
                            $inf_channel = $row["INF_CHANNEL"];
                            $style_new_cst ="";
                            $FILE_CNT1=0;
...
//                             }
							...
							<select style="height:35px;" id="proc_'.$row["BIZ_ID"].'" name="proc_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                                    ...
                                    $proc == "E7208" ? $output .= '<option value="E7208" selected>수임</option>' : $output .='<option value="E7208">수임</option>';
                                    $proc == "E7231" ? $output .= '<option value="E7231" selected>보류</option>' : $output .='<option value="E7231">보류</option>';
                                    $proc == "E7232" ? $output .= '<option value="E7232" selected>카톡</option>' : $output .='<option value="E7232">카톡</option>';
                            ...
                            <td>'.$row["MOBILE"].'</td>
                            <td>
                                <select style="height:35px;" id="inf_channel_'.$row["BIZ_ID"].'" name="inf_channel_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                                    
                                    $inf_channel == "" ? $output .= '<option value="" selected>전체</option>' : $output .='<option value="">전체</option>';
                                    $inf_channel == "채널톡" ? $output .= '<option value="채널톡" selected>채널톡</option>' : $output .='<option value="채널톡">채널톡</option>';
                                    $inf_channel == "전화" ? $output .= '<option value="전화" selected>전화</option>' : $output .='<option value="전화">전화</option>';
                                    $inf_channel == "카톡채널" ? $output .= '<option value="카톡채널" selected>카톡채널</option>' : $output .='<option value="카톡채널">카톡채널</option>';
                                    $inf_channel == "기타" ? $output .= '<option value="기타" selected>기타</option>' : $output .='<option value="기타">기타</option>';
                            ...
                            $output .= '<td>'.$row["HomeTaxID"].'</td>';
                            $output .= '<td>'.$row["HomeTaxPW"].'</td>
                            <td>'.$row["SECTOR"].'</td>
                            <td>'.$row["POP_AMOUNT_PAID"].'</td>

                            <td>
                                <DIV id="pay_lbl_'.$row["BIZ_ID"].'" style="width:100px;height:28px;padding-top:8px;" onclick="javascript:switch_comp2(this)">'.mb_strimwidth($row['EXP_PAY_TAX_SELF'],'0','40','...','utf-8').'</DIV><input style="width:100px; height:28px;display:none;" type="text" id="pay_ip_'.$row["BIZ_ID"].'" value="'.$row['EXP_PAY_TAX_SELF'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)pay_submit(this);" onKeyUp="removeChar(event);inputNumberFormat(this);"></input></TD>
                            </td>
                            <td><a href="javascript:go_cal();">계산</a></td>
                            <td>
                                <select style="height:35px;" id="est_fee_'.$row["BIZ_ID"].'" name="est_fee_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                                    
                            $est_fee_self == "0" ? $output .= '<option value="">선택</option>' : $output .='<option value="">선택</option>';
                            $est_fee_self == "55000" ? $output .= '<option value="55000" selected>55,000</option>' : $output .='<option value="55000">55,000</option>';
                            $est_fee_self == "88000" ? $output .= '<option value="88000" selected>88,000</option>' : $output .='<option value="88000">88,000</option>';
                            $est_fee_self == "110000" ? $output .= '<option value="110000" selected>110,000</option>' : $output .='<option value="110000">110,000</option>';
                            $est_fee_self == "143000" ? $output .= '<option value="143000" selected>143,000</option>' : $output .='<option value="143000">143,000</option>';
                            $est_fee_self == "165000" ? $output .= '<option value="165000" selected>165,000</option>' : $output .='<option value="165000">165,000</option>';
                            $est_fee_self == "198000" ? $output .= '<option value="198000" selected>198,000</option>' : $output .='<option value="198000">198,000</option>';
                            $est_fee_self == "220000" ? $output .= '<option value="220000" selected>220,000</option>' : $output .='<option value="220000">220,000</option>';
                            $est_fee_self == "275000" ? $output .= '<option value="275000" selected>275,000</option>' : $output .='<option value="275000">275,000</option>';
                            $est_fee_self == "330000" ? $output .= '<option value="330000" selected>330,000</option>' : $output .='<option value="330000">330,000</option>';
                            $est_fee_self == "385000" ? $output .= '<option value="385000" selected>385,000</option>' : $output .='<option value="385000">385,000</option>';
                            $est_fee_self == "440000" ? $output .= '<option value="440000" selected>440,000</option>' : $output .='<option value="440000">440,000</option>';
                            $est_fee_self == "495000" ? $output .= '<option value="495000" selected>495,000</option>' : $output .='<option value="495000">495,000</option>';
                            $est_fee_self == "550000" ? $output .= '<option value="550000" selected>550,000</option>' : $output .='<option value="550000">550,000</option>';
                            $est_fee_self == "605000" ? $output .= '<option value="605000" selected>605,000</option>' : $output .='<option value="605000">605,000</option>';
                            $est_fee_self == "660000" ? $output .= '<option value="660000" selected>660,000</option>' : $output .='<option value="660000">660,000</option>';
                            $est_fee_self == "770000" ? $output .= '<option value="770000" selected>770,000</option>' : $output .='<option value="770000">770,000</option>';
                            $est_fee_self == "880000" ? $output .= '<option value="880000" selected>880,000</option>' : $output .='<option value="880000">880,000</option>';
                            $est_fee_self == "990000" ? $output .= '<option value="990000" selected>990,000</option>' : $output .='<option value="990000">990,000</option>';
                            $est_fee_self == "1100000" ? $output .= '<option value="1100000" selected>1,100,000</option>' : $output .='<option value="1100000">1,100,000</option>';
                            $est_fee_self == "1200000" ? $output .= '<option value="1200000" selected>1,200,000</option>' : $output .='<option value="1200000">1,200,000</option>';
                            $est_fee_self == "1300000" ? $output .= '<option value="1300000" selected>1,300,000</option>' : $output .='<option value="1300000">1,300,000</option>';
                            $est_fee_self == "1400000" ? $output .= '<option value="1400000" selected>1,400,000</option>' : $output .='<option value="1400000">1,400,000</option>';
                            $est_fee_self == "1500000" ? $output .= '<option value="1500000" selected>1,500,000</option>' : $output .='<option value="1500000">1,500,000</option>';
                            $est_fee_self == "1600000" ? $output .= '<option value="1600000" selected>1,600,000</option>' : $output .='<option value="1600000">1,600,000</option>';
                            $est_fee_self == "1700000" ? $output .= '<option value="1700000" selected>1,700,000</option>' : $output .='<option value="1700000">1,700,000</option>';
                            $est_fee_self == "1800000" ? $output .= '<option value="1800000" selected>1,800,000</option>' : $output .='<option value="1800000">1,800,000</option>';
                            $est_fee_self == "1900000" ? $output .= '<option value="1900000" selected>1,900,000</option>' : $output .='<option value="1900000">1,900,000</option>';
                            $est_fee_self == "2000000" ? $output .= '<option value="2000000" selected>2,000,000</option>' : $output .='<option value="2000000">2,000,000</option>';
                            $est_fee_self == "2100000" ? $output .= '<option value="2100000" selected>2,100,000</option>' : $output .='<option value="2100000">2,100,000</option>';
                            $est_fee_self == "2100000" ? $output .= '<option value="2200000" selected>2,200,000</option>' : $output .='<option value="2200000">2,200,000</option>';
                            $est_fee_self == "-9999" ? $output .= '<option value="-9999" selected>별도협의</option>' : $output .='<option value="-9999">별도협의</option>';
                                    $output .='</select>
                            </td>
                            <td>
							<select style="height:35px;" id="send_kakao_'.$row["BIZ_ID"].'" name="send_kakao_'.$row["BIZ_ID"].'">';
                                    
                                    $output .= '<option value="bizp_2022042114014314053182657" selected>수수료 안내</option>';
                                    $output .= '<option value="bizp_2022042113590613488325669">세액  & 수수료 안내</option>';
                                    
                                    $output .= '<option value="bizp_2022042114030514053585658">예상세액 안내</option>';
                                    $output .= '<option value="bizp_2022042114073514053486659">부재중전화 안내</option>';
                                    $output .= '<option value="bizp_2022042114114213488987672">홈택스 안내문 조회방법</option>';
                                    $output .= '<option value="bizp_2022042114131114053954660">신고마감임박 안내</option>';
                                    $output .= '<option value="bizp_2022042114151414053069663">수수료 조정 안내</option>';
                                    
                                    $output .='</select>
							</td>
                            
                            <td>';
                                    $kakao_2 ==""?$output.='<a href="javascript:send_kakao_pro(\''.$row["ID"].'\',\''.$row["BIZ_ID"].'\' , \''.$row["MOBILE"].'\');">발송</a>' : $output.='발송';
                                    $output .='</td>
                            <td><a href="javascript:window.open(\'list_kakao_send_pro.php?biz_id='.$row["BIZ_ID"].'\');">'.$cnt_kakao.'</a></td>
							<td>'.$row["REGDATE_"].'</td>
							<td>
		                    <select style="height:35px;" id="bran_'.$row["BIZ_ID"].'" name="bran_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
							...
							<select style="height:35px;" id="reguser_'.$row["BIZ_ID"].'" name="reguser_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                                    ...
                                    if($bran == "D1019"){
                                        $reguser == "" ? $output .= '<option value="" selected>선택</option>' : $output .='<option value="">선택</option>';
                                        $reguser == "1231" ? $output .= '<option value="1231" selected>신승01</option>' : $output .='<option value="1231">신승01</option>';
                                        $reguser == "1232" ? $output .= '<option value="1232" selected>신승02</option>' : $output .='<option value="1232">신승02</option>';
                                        $reguser == "1233" ? $output .= '<option value="1233" selected>신승03</option>' : $output .='<option value="1233">신승03</option>';
                                        $reguser == "1234" ? $output .= '<option value="1234" selected>신승04</option>' : $output .='<option value="1234">신승04</option>';
...
//  종소세_접수현황 진행상태 select option값 로딩 : 시작
if ($action == "select_option_value_") {
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_TB750010(IN P_OPT VARCHAR(50))
	BEGIN
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB750010"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query = "CALL SELECT_TB750010('".$opt."')";
                $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
                
                if(mysqli_num_rows($result) >0)
                {
                    $output .= '<option value="">'.$opt.'-전체</option>';
                    while($row = mysqli_fetch_array($result)){
...
// 종소세_접수현황 진행상태 select option값 로딩: 끝




//  메뉴팝업_LEFT_개인영역 : 시작
if ($action == "select_menu_pop_my") {
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_TB600050_MY(IN P_USERID INT(11))
	BEGIN
    ...
    FROM TB600050 
    WHERE CATE = 'MY'
    AND IND_USERID = P_USERID
    ORDER BY CATE, MENU_IDX
	;
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB600050_MY"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query = "CALL SELECT_TB600050_MY(".$userid.")";
                $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
                $cnt_result = mysqli_num_rows($result);
                $counter = 0;
                if(mysqli_num_rows($result) >0)
                {
                    $temp_cate = "";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($temp_cate != $cate && $temp_cate != "")
                            $output .='</div>';
                            
                        $temp_cate = $cate;
                        
                        if($menu_idx == $first_idx)
                            $output .='<ul id="BTN_'.$cate.'" style="'.$css.'"><li class="title">MY 답변</li>';
                        
                            
                        //$output .= '<DIV id="MENU_IDX_'.$menu_idx.'" value="'.$row["ID"].'" onclick="load_content(this);">'.$row["MENU_NAME"].'</DIV>';
                            $output .= '<li id="MENU_IDX_'.$menu_idx.'" value="'.$row["ID"].'" onclick="load_content(this);"><a>'.$row["MENU_NAME"].'<strong onclick="javascript:cp_content('.$row["ID"].');"></strong></a></li>';
                        
                        
                        if(++$counter == $cnt_result){
                            $output .= '<li id="MENU_IDX_NEW" value="new" onclick="add_menu();"><a>문구추가<strong></strong></a></li>';
                            $output .= '</ul>';
                        }
                        $tmp_i++;
                    ...
                    $output .='<ul id="BTN_MY" style="display:none;"><li class="title">MY 답변</li>';
                    $output .= '<li id="MENU_IDX_NEW" value="new" onclick="add_menu();"><a>문구추가<strong></strong></a></li>';
                    $output .= '</ul>';
                }
...
//  메뉴팝업_LEFT : 시작
if ($action == "select_menu_pop") {
    
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_TB600050()
	BEGIN
            
    SELECT
        *
    FROM TB600050
WHERE IFNULL(IND_USERID,'') = ''    
ORDER BY CATE, MENU_IDX
	;
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB600050"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query = "CALL SELECT_TB600050()";
                $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
                $cnt_result = mysqli_num_rows($result);
                $counter = 0;
                if(mysqli_num_rows($result) >0)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($menu_idx == "1"){
                            $output .='<ul id="BTN_'.$cate.'" style="'.$css.'"><li class="title">자주쓰는 답변</li>';
                            //$output .='<div id="BTN_'.$cate.'" style="'.$css.'" >';
                        }
                        //$output .= '<DIV id="MENU_IDX_'.$menu_idx.'" value="'.$row["ID"].'" onclick="load_content(this);">'.$row["MENU_NAME"].'</DIV>';
                        $output .= '<li id="MENU_IDX_'.$menu_idx.'" value="'.$row["ID"].'" onclick="load_content(this);"><a>'.$row["MENU_NAME"].'<strong onclick="javascript:cp_content('.$row["ID"].');"></strong></a></li>';
...
if ($action == "select_content") {
    ...
    if(isset($_POST["action"]))
    {
        ...
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_TB600050_CONTENT(IN P_ID INT(11))
	BEGIN
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB600050_CONTENT"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query = "CALL SELECT_TB600050_CONTENT('".$id."')";
                $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
...
if ($action == "select_family") {
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_TB310040( IN P_ID INT(11) )
	BEGIN
            
    SELECT 
        ID,
        FN_FAMILY_RELATION(RELATION) AS 'RELATION', FAM_NAME, RESIDENT_ID, 
        FN_FAMILY_DISALBED(DISORDER) AS 'DISORDER', 
        FN_NUMBER_TO_YN(WOMAN) AS 'WOMAN', 
        FN_NUMBER_TO_YN(SINGLE_PARENT) AS 'SINGLE_PARENT',
        FN_NUMBER_TO_YN(IS_SCHOOL) AS 'IS_SCHOOL',
        FN_NUMBER_TO_YN(BIRTH_ADOPTION) AS 'BIRTH_ADOPTION'
    FROM TB310040 WHERE CSTID = P_ID
    ORDER BY RELATION
	;
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB310040"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query = "CALL SELECT_TB310040('".$id."')";
                $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
                    ...
                    while($row = mysqli_fetch_array($result)){
                        $output .= '
        					<tr style="">
                                ...
                                <td >'.$row["BIRTH_ADOPTION"].'</td>
                                <td >'.$row["IS_SCHOOL"].'</td>
                                <td ><a href="javascript:del('.$row["ID"].');">삭제</a></td>
                ...
                else
                {
                    $output .= '
				<tr>
				<td colspan="7" style="text-align:center;">등록된데이터가 없습니다.</td>
				</tr>
...
// 재산제세>글상세>코멘트 select 
if($action == "select_comment"){
    $id=mysqli_real_escape_string($connect,$_POST["id"]);
    $userid=mysqli_real_escape_string($connect,$_POST["userid"]);
    $b_flag=mysqli_real_escape_string($connect,$_POST["b_flag"]);
    $mobile_flag = mysqli_real_escape_string($connect,$_POST["mobile_flag"]);
    ...
    $head .= '
			...
			';
    $procedure = "
	CREATE PROCEDURE SELECT_COMMENT(IN P_ID INT, P_BOARD_FLAG VARCHAR(5) )
	BEGIN
		SELECT *,SELECT_REGUSER(A.REGUSER) 'REGUSER_' ,DATE_FORMAT(A.REGDATE, '%y-%m-%d') 'REGDATE_' ,CODE_TO_STR(B.DEPID) 'DEPID_'
        FROM TB600042 AS A LEFT OUTER JOIN TB980010 AS B
        ON A.REGUSER = B.USERID AND B.USERID = A.REGUSER
        WHERE A.CSTID = P_ID
        AND VISIBLE = 'Y'
		AND BOARD_FLAG = P_BOARD_FLAG
        ORDER BY A.REGDATE DESC;
	END;
	";
    

    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_COMMENT"))
    {
        if(mysqli_query($connect,$procedure))
        {
            $query = "CALL SELECT_COMMENT(".$id.", '".$b_flag."')";
            $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
                    $reguser = $row["REGUSER"];
                    if($reguser == $userid){
                        $del_str = '<a style="padding-left:10px;padding-top:0px;" href="javascript:del_cmt('.$row["ID"].');">삭제</a>';
                    }else{
                        $del_str = "";
                    }
                    if($mobile_flag <> 'm'){
                        ...
                        $output .= '
        				<tr>
                            <td >'.$row["REGUSER_"].'('.$row["DEPID_"].')</td>
                            <td >'.$row["COMMENT"].'<span style="padding-left: 10px;    font-size: 13px;    color: #b0b0b0;"></span>
                            '.$del_str.'
                            </TD>
        				</tr>
        				';
                        ...
                        $output .= '
        				<tr>
                            <td rowspan=2>'.$row["REGUSER_"].'('.$row["DEPID_"].')</td>
                                                    
                            <td style="text-align:left;padding: 15px;"><div style="word-break: break-all;">'.$row["COMMENT"].'</div><span style="padding-left: 10px;    font-size: 13px;    color: #b0b0b0;"></span>
                            </TD>
        				</tr>
                        <tr>
                            <td><div style="margin-top:5px;">등록일 : '.$row["REGDATE"].''.$del_str.'</div></td>
                        </tr>
        				';
                    }
...
// 알림톡 발송 대상자 그룹
if($action == "select_list_relation_setting"){
    $master = '';
    $head .= '
			<colgroup>
				<col width="200px">
	...
	BEGIN
		#SELECT A.USERNAME, SELECT_REGUSER(A.USERID) AS USERID, SELECT_REGUSER(B.M_MASTER) AS 'MASTER', DATE_FORMAT(B.EDTDATE, '%y-%m-%d') 'EDTDATE_'
        #FROM TB980010 AS A LEFT OUTER JOIN TB980093 AS B ON A.USERID = B.M_SLAVE
        #WHERE DEPID = 'D1013';
        
        SELECT A.ID,
        SELECT_REGUSER(A.M_MASTER) AS 'M_MASTER',
        SELECT_REGUSER(A.M_SLAVE) AS 'M_SLAVE' ,
        DATE_FORMAT(A.REGDATE, '%y-%m-%d') 'REGDATE_' 
        FROM TB980093 AS A LEFT OUTER JOIN TB980010 AS B ON A.M_SLAVE = B.USERID
        WHERE B.DEPID = 'D1013';
	END;
	";
            ...
            if(mysqli_query($connect,$procedure))
            {
                ...
                if(mysqli_num_rows($result) >0)
                {
					...
					<tr style="text-align:center; padding:20px 20px 10px 20px;">
                        ...
                        <td >'.$row["M_MASTER"].'</td>
                        <td >'.$row["REGDATE_"].'</td>
                        <TD><a href="javascript:del_relation('.$row["ID"].');">삭제</a></TD>
					</tr>
					';
                    }
                }
                else
                {
                    $output .= '
				<tr>
				<td colspan="3" style="text-align:center;">등록된데이터가 없습니다.</td>
				</tr>
...
//멤버리스트_알림톡발송 대상자 : 시작
if($action == "select_memberlist_group"){
    ...
    if($_POST["s_option"] != ""){
        switch($s_option){
            case "USERNAME" :
                $WHERE_STR .= " AND A.USERNAME like '%".$s_str."%' ";
                break;
            case "DEPNAME" :
                $WHERE_STR .= " AND B.BALUE_ like '%".$s_str."%' ";
                break;
            case "POSITION" :
                $WHERE_STR .= " AND C.VALUE_ like '%".$s_str."%' ";
                break;
            default:
                $WHERE_STR ="";
        }
    ...
    $procedure = "
	CREATE PROCEDURE SELECT_MEMBER_LIST(IN P_ID INT)
	BEGIN
        ...
        AND IFNULL(MOBILE,'') <> ''
        AND USERID NOT IN (SELECT USERID FROM TB600040 WHERE GROUP_ID = P_ID)
		ORDER BY DEPID, INNER_PHONE;
	END;
	";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_MEMBER_LIST"))
    {
        if(mysqli_query($connect,$procedure))
        {
            $query = "CALL SELECT_MEMBER_LIST('".$_POST["id"]."')";
            $result = mysqli_query($connect,$query);
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
                    ...
                    $output .= '
					<tr>
					<td><input type="checkbox" name="check[]" class="chk" value="'.$row["USERID"].'"></td>
					<td>'.$row["USERNAME"].'</td>
					<td>'.$row["DEPNAME"].'</td>
					<td>'.$row["POSITION"].'</td>
					</tr>
...
// 알림톡 발송 대상자 그룹
if($action == "select_list_kakao_option"){
    ...
    $head .= '
			...
			';
    $procedure = "
	CREATE PROCEDURE SELECT_LIST_KAKAO_OPTION()
	BEGIN
		SELECT *, CODE_TO_STR_ETC(USE_MENU_ID) AS 'USE_MENU_' 
        FROM TB600041 ORDER BY ID DESC;
	END;
	";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_LIST_KAKAO_OPTION"))
    {
        if(mysqli_query($connect,$procedure))
        {
            $query = "CALL SELECT_LIST_KAKAO_OPTION()";
            $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
					...
					<tr style="text-align:center; padding:20px 20px 10px 20px;">
                        ...
                        <td >'.$row["USE_YN"].'</td>
                        <td >'.$row["ETC"].'</td>
                        <td ><button class="b_search" id="btn_add_member" name="btn_add_member" onClick="javascript:add_member('.$row["GROUP_ID"].');">멤버추가</button></td>
					</tr>
					';
                }
            }
...
if($action == "select_disc_list"){
	...
	BEGIN
		SELECT *, 
CODE_TO_STR(BRANCH) AS BRANCH_,
CODE_TO_STR(TOTAL_EVAL) AS TOTAL_EVAL_

        FROM TB980090 ORDER BY USERID DESC;
	END;
	";
        ...
        if(mysqli_query($connect,$procedure))
        {
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
					...
					<tr style="text-align:center; padding:20px 20px 10px 20px;">
                        <td >'.$row["USERID"].'</td>
                        <td ><a href="view_disc.php?id='.$row["USERID"].'">'.$row["USERNAME"].'</a></td>
<TD>'.$row["BRANCH_"].'</TD>
<TD>'.$row["MOBILE"].'</TD>
<TD>'.$row["TOTAL_EVAL_"].'</TD>
                        <td ><a href="write_disc.php?id='.$row["USERID"].'">링크</a></td>
                        <td >'.$row["REGDATE"].'</td>
                        <td >'.$row["G1_TYPE"].'</td>
                        <td >'.$row["G2_TYPE"].'</td>
                        <td><b>'.$row["G3_TYPE"].'</b></td>
                        <td >'.$row["REP_FLAG"].'</td>
<TD>'.$row["INTERVIEW_COMMENT"].'</TD>
					</tr>
					';
                }
            }
...
if($action == "select_disc_list_m"){
	...
	CREATE PROCEDURE SELECT_DISC_LIST()
	BEGIN
		SELECT CODE_TO_STR(PROGRESS) 'PROGRESS_',USERID,  USERNAME, TOTAL_CAR_YEAR, TOTAL_CAR_MONTH,
        CODE_TO_STR(BRANCH) 'BRANCH_', HOPE_MONEY, CODE_TO_STR(TOTAL_EVAL) 'TOTAL_EVAL_' 
        FROM TB980090 ORDER BY USERID DESC;
	END;
	";
        ...
        if(mysqli_query($connect,$procedure))
        {
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
					...
					<tr style="text-align:center; padding:20px 20px 10px 20px;">
                        <td >'.$row["PROGRESS_"].'</td>
                        <td ><a href="view_disc.php?id='.$row["USERID"].'">'.$row["USERNAME"].'</td>
                        <td >'.$row["TOTAL_CAR_YEAR"].'년<BR>'.$row["TOTAL_CAR_MONTH"].'개월</td>
                        <td >'.$row["BRANCH_"].'</td>
                        <td >'.$row["HOPE_MONEY"].'</td>
                        <td >'.$row["TOTAL_EVAL_"].'</td>
            ...
            else
            {
                $output .= '
				<tr>
				<td colspan="3" style="text-align:center;">�뜲�씠�꽣媛� �뾾�뒿�땲�떎.</td>
				</tr>
...
if($action == "select_ratio"){
    ...
    $procedure = "
		CREATE PROCEDURE `SELECT_USER_RATIO`(IN P_CSTID INT )
BEGIN

	SELECT USER_RATIO FROM TB100022 WHERE CSTID = P_CSTID AND CST_TYPE='A1001' AND CST_TYPE_YEAR = $SET_YEAR
    ;
END
		";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_USER_RATIO"))
    {
        //mysqli_query:DB에 쿼리 전송
        if(mysqli_query($connect,$procedure))
        {
            $query = "CALL SELECT_USER_RATIO('".$CSTID."')";
            $result = mysqli_query($connect,$query);
            ///mysqli_query:DB에 쿼리 전송 후 결과값을 변수에 담는다
            
            while($row = mysqli_fetch_array($result)){
                $output["USER_RATIO"] = $row["USER_RATIO"];
            }
...
// 종합소득세 장부계산프로그램 select : 시작
if($action == "select_income_cal"){
    ...
    $procedure = "
		CREATE PROCEDURE `SELECT_INCOME_CAL`(IN P_CSTID INT, P_USER_RATIO INT )
BEGIN

	DECLARE TMP_STEP1 INT;
	DECLARE TMP_STEP2 INT;
	DECLARE TMP_STEP3 INT;
	DECLARE TMP_STEP4 VARCHAR(10);
	DECLARE TMP_STEP5 INT;
	DECLARE TMP_STEP6 INT;
    DECLARE TMP_STEP10 INT;
    
        
	SET TMP_STEP1 = CEIL(IFNULL(CAL_STEP1_NEW(P_CSTID), 0) * (P_USER_RATIO/100));
	SET TMP_STEP2 = IFNULL(TMP_STEP1 - CAL_STEP10_NEW(P_CSTID) , 0);
	SET TMP_STEP3 = IFNULL(CAL_STEP3_NEW(TMP_STEP2), 0);
	SET TMP_STEP4 = IFNULL(CAL_STEP3_TAX_AVR(TMP_STEP2), 0);
	SET TMP_STEP5 = IFNULL(TMP_STEP3 - CAL_STEP11_NEW(P_CSTID) ,0);
	SET TMP_STEP6 = IFNULL(TMP_STEP5 - IFNULL(CAL_STEP4_NEW(P_CSTID),0) , 0);
        
	SELECT FORMAT( TMP_STEP1 ,0) AS 'STEP1' ,
	FORMAT( TMP_STEP2 ,0) AS 'STEP2',
	FORMAT(TMP_STEP3,0) AS 'STEP3',
	TMP_STEP4 AS 'STEP4', 
	FORMAT(TMP_STEP5,0) AS 'STEP5',
	FORMAT( IFNULL(CAL_STEP4_NEW(P_CSTID),0) ,0) AS 'STEP8', 
	FORMAT(TMP_STEP6 , 0) AS 'STEP9',
    FORMAT(CAL_STEP10_NEW(P_CSTID) , 0) AS 'STEP10',
    FORMAT(CAL_STEP11_NEW(P_CSTID) , 0) AS 'STEP11',
    (SELECT USER_RATIO FROM TB100022 WHERE CSTID = P_CSTID AND CST_TYPE='A1001' AND CST_TYPE_YEAR = $SET_YEAR) AS 'USER_RATIO'
    ;
END
		";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_INCOME_CAL"))
    {
        //mysqli_query:DB에 쿼리 전송
        if(mysqli_query($connect,$procedure))
        {
            $query = "CALL SELECT_INCOME_CAL('".$CSTID."','".$user_ratio."')";
            $result = mysqli_query($connect,$query);
            ///mysqli_query:DB에 쿼리 전송 후 결과값을 변수에 담는다
            
            while($row = mysqli_fetch_array($result)){
                $output['STEP1'] = $row["STEP1"];
                $output['STEP2'] = $row["STEP2"];
                $output['STEP3'] = $row["STEP3"];
                $output['STEP4'] = $row["STEP4"];
                $output['STEP5'] = $row["STEP5"];
                $output['STEP6'] = $row["STEP5"];
                $output['STEP8'] = $row["STEP8"];
                $output['STEP9'] = $row["STEP9"];
                $output['STEP10'] = $row["STEP10"];
                $output['STEP11'] = $row["STEP11"];
                $output['USER_RATIO'] = $row["USER_RATIO"];
...
// 그룹멤버리스트 select : 시작
if($action == "select_group_member_list"){
    $id=mysqli_real_escape_string($connect,$_POST["id"]);
    $member_list = "";
...
<th>이름</th>
    ...
    $procedure = "
		CREATE PROCEDURE SELECT_MEMBER_LIST_KAKAO(IN P_GROUP_ID INT )
		BEGIN
        
            SELECT A.USERID,RETURN_USERID_TO_REGUSER(A.USERID) 'USERNAME', DATE_FORMAT(A.REGDATE, '%y-%m-%d') 'REGDATE_'
            FROM TB600040 AS A LEFT OUTER JOIN TB980010 AS B ON A.USERID = B.USERID
            WHERE GROUP_ID = P_GROUP_ID; 
        
		END;
		";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_MEMBER_LIST_KAKAO"))
    {
        //mysqli_query:DB에 쿼리 전송
        if(mysqli_query($connect,$procedure))
        {
            $query1 = "CALL SELECT_MEMBER_LIST_KAKAO('".$_POST["id"]."')";
            $result = mysqli_query($connect,$query1);
            ...
            while($row = mysqli_fetch_array($result)){
                ...
                $member_list .= '
					<tr style="text-align:center; padding:20px 20px 10px 20px;">
                        ...
                        <td >'.$row["USERNAME"].'</td>
                        <td >'.$row["REGDATE_"].'</td>
                        <td ><a href="javascript:mem_del('.$row["USERID"].');">삭제</a> </td>
					</tr>
					';
            }
            
            echo $head.$member_list;
...
if($action == "select_list_reginfo"){
 ...
 WHERE B.CST_TYPE = 'A1001' ".$query_str1.$query_str2.$query_str3;
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_REGINFO_ICM()
	BEGIN
	SELECT
    A.CSTID AS 'ID',
    CODE_TO_STR(B.REG_BRANCH) AS 'REG_BRANCH_PATH',
...
substring( REPLACE(A.MOBILE,'-',''),4) MOBILE_PATH,
    ...
    CK_KAKAO_SEND_CNT(B.BIZ_ID, REPLACE( A.MOBILE,'-',''), 'A1001', 'STEP02') AS 'KAKAO_SEND_2',
    B.INF_CHANNEL,
	IF(IFNULL(B.INF_PATH,'')<>'',B.INF_PATH, B.INF_CHANNEL) AS INF_PATH,
    B.POP_AMOUNT_PAID
	...
	";
        ...
        $hostname=$_SERVER["HTTP_HOST"];
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_REGINFO_ICM"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_REGINFO_ICM()";
                ...
                if($result)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
                            ...
                            <td>'.$row["MOBILE"].'</td>
                            <td>
                                <select style="height:35px;" id="inf_path_'.$row["BIZ_ID"].'" name="inf_path'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                                    $DOWN_URL_PAY == "" ? $output .= '<option value="" selected>선택</option>' : $output .='<option value="">선택</option>';
                                    $DOWN_URL_PAY== "전문상담" ? $output .= '<option value="전문상담" selected>전문상담</option>' : $output .='<option value="전문상담">전문상담</option>';
                                    $DOWN_URL_PAY== "전화" ? $output .= '<option value="전화" selected>전화</option>' : $output .= '<option value="전화">전화</option>';
                                    $DOWN_URL_PAY== "종소톡" ? $output .= '<option value="종소톡" selected>종소톡</option>' : $output .= '<option value="종소톡">종소톡</option>';
                                    $DOWN_URL_PAY== "대량업로드" ? $output .= '<option value="대량업로드" selected>대량업로드</option>' : $output .= '<option value="대량업로드">대량업로드</option>';
...
/*부가세 영업현황 select : 시작*/
if($action == "select_list_reginfo_VAT"){
	...
	LEFT OUTER JOIN TB980010 AS D ON D.USERID = B.DEC_REGUSER
	LEFT OUTER JOIN TB300031 AS E ON A.CSTID=E.CSTID
 WHERE B.CST_TYPE = '".$cst_type."' ".$query_str1.$query_str2.$query_str3;
...
substring( REPLACE(A.MOBILE,'-',''),4) MOBILE_PATH,
    ...
    #CAL_FEE_CHANGE_DATE(A.CSTID) AS 'CAL_EST_FEE_FN',
    NEW_CST_CK(A.CSTID) AS 'NEW_CST_CK',
    CK_KAKAO_SEND_CNT(B.BIZ_ID, REPLACE( A.MOBILE,'-',''), 'A1008', '부가세알림톡발송1') AS 'KAKAO_SEND_1',
    CK_KAKAO_SEND_CNT(B.BIZ_ID, REPLACE( A.MOBILE,'-',''), 'A1008', '부가세알림톡발송2') AS 'KAKAO_SEND_2',
    B.INF_CHANNEL,
	IF(IFNULL(B.INF_PATH,'')<>'',B.INF_PATH, B.INF_CHANNEL) AS INF_PATH
	FROM TB100020 AS A
	LEFT OUTER JOIN TB100022 AS B ON A.CSTID=B.CSTID
        ...
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_REGINFO"))
        {
            ...
            if(mysqli_query($connect,$procedure))
            {
                ...
                if($result)
                {
                    ...
                    $cate_name="";
                    $path_pay = "../tax_vat/upload/$SET_YEAR/";
                    //$path_info = "upload_income/2021/";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        //if($cst_type == "A1001"){
                            ...
                            <td>
                                    <DIV id="name_lbl_'.$row["ID"].'" style="width:80px;height:28px;padding-top:8px;" onclick="javascript:switch_name(this)">'.$SEND_NAME .'</DIV><input style="width:80px; height:28px;display:none;" type="text" id="name_ip_'.$row["ID"].'" value="'.$SEND_NAME .'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)name_submit(this);" ></input>
                            </td>
                            <td>'.$row["CST_TYPE_YEAR"].'</td>
                            <td>'.$row["MOBILE"].'</td>
                            <td>'.$DOWN_URL_PAY.'</td>
                            <td><select style="height:35px;'.$style_new_cst.'" id="new_cst_'.$row["ID"].'" name="new_cst_'.$row["ID"].'" onchange="javascript:modify_option(this);">';
                                    ...
                                    $output .='</select></td>
                            <td>';
                                    $kakao_1 == 0 ?$output.='<a href="javascript:send_kakao(\'self_1\',\''.$row["ID"].'\' ,\''.$row["BIZ_ID"].'\' ,\''.$SEND_NAME.'\');">
                                    <span data-tooltip-text="#{고객명}님 
...
신고담당자가 수수료 입금계좌 및 필요서류를 안내해드리겠습니다.">
                            ...
                            <td>'.$kakao_1.'</td>
                            <td>';
                                    $kakao_2 ==""?$output.='<a href="javascript:send_kakao(\'self_2\',\''.$row["ID"].'\',\''.$row["BIZ_ID"].'\' ,\''.$SEND_NAME.'\');">
<span data-tooltip-text="#{고객명}님 
...
if($action=="select_list_acc"){
    ...
    $e_option4 = $_POST["e_option4"];
    $prog_option = $_POST["prog_option"];
    $isnew_option = $_POST["isnew_option"];
    $cashreport_option = $_POST["cashreport_option"];
    $agree_option = $_POST["agree_option"];
    $reg_option = $_POST["reg_option"];
    $login_review_option = $_POST["login_review_option"];
    ...
    $s_date1_from = $_POST["s_date1_from"];
    $s_date1_to = $_POST["s_date1_to"];
    
    $s_date2_from = $_POST["s_date2_from"];
    $s_date2_to = $_POST["s_date2_to"];
    
    $step_name_search=$_POST["step_name_flag"];
    $sort = $_POST["sort_flag"];
	...
	LEFT OUTER JOIN TB980010 AS D ON D.USERID = B.DEC_REGUSER
    LEFT OUTER JOIN TB980010 AS D2 ON D2.USERID = B.REGUSER
    LEFT OUTER JOIN TB100026 AS E ON B.BIZ_ID = E.BIZ_ID AND E.PROGRESS IN ('E7208','E7216','E7217','E7218','E7219','E7220','E7221','E7222','E7223','E7224','E7225','E7226','E7227','E7228','E7229','E7230','E7232')
     
	WHERE 1=1
    AND IFNULL(E.PROGRESS ,'') <> ''
    AND B.CST_TYPE = 'A1001' ".$query_date.$query_year.$query_str1.$query_str2.$query_str3.$query_str4.$query_rv;
...
<col width="70px">
		...
		<thead>
        <tr><td  colspan=34><b>총 '.$total.' 건이 검색되었습니다. </b></td></tr>
		<tr>
			<th>진행상태</th>
			<th>이름</th>
			...
			<th>예상수수료</th>
            <th>입금금액</th>
			<th colspan=2><a href="javascript:sorting(\'CashReport\',\''.$sort.'\');" style="padding:3px;"><b>현금영수증</b></a>'.$sort_css_cr.'</th>
			<th colspan=2><a href="javascript:sorting(\'HomeTaxConsignment\',\''.$sort.'\');" style="padding:3px;"><b>수임동의</b></a>'.$sort_css_hc.'</th>
            <th colspan=3><a href="javascript:sorting(\'HomeTaxPrint\',\''.$sort.'\');" style="padding:3px;"><b>자료추출</b></a>'.$sort_css_hp.'</th>
            <th>조정</th>
            <th colspan=3><a href="javascript:sorting(\'SmartABookMake\',\''.$sort.'\');" style="padding:3px;"><b>자동 간편장부</b></a>'.$sort_css_bm.'</th>
            <th>지점</th>
			<th>작성자</th>
            <th>실작성자</th>
            <th>서버</th>
            <th>코드</th>
            <th colspan=2><a href="javascript:sorting(\'SmartAToConvert\',\''.$sort.'\');" style="padding:3px;"><b>파일변환</b></a>'.$sort_css_sc.'</th>
            <th colspan=3><a href="javascript:sorting(\'HomeTaxUpload\',\''.$sort.'\');" style="padding:3px;"><b>전자신고</b></a>'.$sort_css_hu.'</th>
<th colspan=2>수동신고</th>
<th colspan=2>알림톡</th>
            <th>신고확인</th>
			<th>접수메모</th>
    ...
    STEP_NAME( F.WehagoBookMake ) AS WEHAGO_BOOK,
    STEP_NAME( F.ManualDown ) AS MENUAL_DOWN,
    SELECT_ERROR_MSG(A.CSTID, 'ManualDown' ) AS ERROR_MSG_MENUAL_DOWN,
    #SELECT_ERROR_MSG(A.CSTID, ) AS ERROR_MSG,
    #(SELECT LOG AS RESULT FROM TB700020 WHERE CSTID = A.CSTID AND STEP_NAME <>'LOGIN'  ORDER BY LOG_TIME DESC LIMIT 1) AS ERROR_MSG,
    (SELECT COUNT(1) FROM TB700010 WHERE SEND_BIZ_ID = B.BIZ_ID AND SEND_TMP_STEP='AUTO_COMPLATE') AS CNT_KAKAO_SEND,
    B.RP_SEND_KAKAO,
                ...
                if($result)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
                            ...
                            //$kakao_2 = $row["KAKAO_SEND_2"];
                            $cash_report=$row["CASH_REPORT"];
                            $ERR_MSG_cash_report=$row["ERROR_MSG_CASH_REPORT"];
                            $smartA=$row["SMARTA"];
                            $ERR_MSG_smartA=$row["ERROR_MSG_SMARTA"];
                            $ht_upload=$row["HT_UPLOAD"];
                            $ERR_MSG_ht_upload=$row["ERROR_MSG_HT_UPLOAD"];
                            $ht_print=$row["HT_PRINT"];
                            $ERR_MSG_ht_print=$row["ERROR_MSG_HT_PRINT"];
                            //$ht_down=$row["HT_DOWNLOAD"];
                            $compreg=$row["COMPREG"];
                            $taxinvoice=$row["TAXINVOICE"];
                            $consign=$row["CONSIGN"];
                            $ERR_MSG_consign=$row["ERROR_MSG_CONSIGN"];
                            $confirm=$row["CONFIRM"];
                            $smarta_book=$row["SMARTA_BOOK"];
                            $ERR_MSG_smarta_book=$row["ERROR_MSG_SMARTA_BOOK"];
                            $wehago_book=$row["WEHAGO_BOOK"];
                            $ERR_MSG_menual_down=$row["ERROR_MSG_MENUAL_DOWN"];
                            $menual_down=$row["MENUAL_DOWN"];
                            ...
                            $FILE_CNT1=0;
                            $FILE_CNT2=0;
                            $ck_login_review = $row["CK_LOGIN_REVIEW"];
                            $user_ratio = $row["USER_RATIO"];
                            
                            if($ck_login_review == "review"){
                                $str_login_review = "리뷰";
                            }elseif($ck_login_review == "login_ok"){
                                $str_login_review = "접속";
                            }else{
                                $str_login_review = "미접속";
                            }
                            
                            
                            //$DOWN_PATH_PAY = $path.$row["CSTNAME"]."_".$row["MOBILE"]."/";
                            $DOWN_PATH_PAY = $path."강남/납부서/".$row["CSTNAME"].$row["MOBILE_PATH"]."/";
                            $DOWN_PATH_INFO = $path."강남/안내문/".$row["CSTNAME"].$row["MOBILE_PATH"]."/";
                            /*
                             $DOWN_PATH_PAY = $path_pay.$row["CSTNAME"]."_".$row["MOBILE"]."/";
                             $DOWN_PATH_INFO = $path_info.$row["CSTNAME"]."_".$row["MOBILE"]."/";
                             */
                            ...
                            //$down_dir = "../tax_income/upload/".$row["CSTNAME"]."_"select_view_ext.$row["MOBILE"]."/";
                            //$dir1 = iconv("UTF-8","CP949",$DOWN_PATH_PAY);
                            //$dir2 = iconv("EUC-KR","UTF-8",$DOWN_PATH_INFO);
                            //if($hostname=="localhost")
                            $dir1 = $DOWN_PATH_INFO;
                            //else
                            //  $dir1 = iconv("UTF-8","CP949",$DOWN_PATH_INFO);
                            ...
                            if($FILE_CNT1 > 0){
                                $DOWN_URL_INFO = "<a href='javascript:open_file_info(".$row["ID"].", $year);'>안내문</a>";
                            }else{
                                $DOWN_URL_INFO = "";
                            }
                            ...
                            if($FILE_CNT2 > 0){
                                $DOWN_URL_PAY = "<a href='javascript:open_file_pay(".$row["ID"]." , $year );'>납부서</a>";
                            }else{
                                $DOWN_URL_PAY = "";
                            }
                            ...
                            if($cash_report=='완료')$cash_style="style='padding-left:6px;background-color:green;color:white;'";
                            if($cash_report=='실행중')$cash_style="style='padding-left:6px;background-color:#c57900;color:white;'";
                            if($cash_report=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($smartA=='에러')$smarta_style="style='background-color:#d10000;color:white;'";
                            if($smartA=='예약')$smarta_style="style='background-color:#007cc5;color:white;'";
                            if($smartA=='완료')$smarta_style="style='background-color:green;color:white;'";
                            if($smartA=='실행중')$smarta_style="style='background-color:#c57900;color:white;'";
                            if($smartA=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($ht_upload=='에러')$upload_style="style='background-color:#d10000;color:white;'";
                            if($ht_upload=='예약')$upload_style="style='background-color:#007cc5;color:white;'";
                            if($ht_upload=='완료')$upload_style="style='background-color:green;color:white;'";
                            if($ht_upload=='실행중')$upload_style="style='background-color:#c57900;color:white;'";
                            if($ht_upload=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($ht_print=='에러')$print_style="style='background-color:#d10000;color:white;'";
                            if($ht_print=='예약')$print_style="style='background-color:#007cc5;color:white;'";
                            if($ht_print=='완료')$print_style="style='background-color:green;color:white;'";
                            if($ht_print=='실행중')$print_style="style='background-color:#c57900;color:white;'";
                            if($ht_print=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($cnt_kakao_send > 0) $style_kakao_send="style='background-color:#007cc5;color:white;text-align:center;padding:0;'";
                            ...
                            if($consign=='완료')$consign_style="style='background-color:green;color:white;'";
                            if($consign=='실행중')$consign_style="style='background-color:#c57900;color:white;'";
                            if($consign=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            ...
                            if($smarta_book=='완료')$smarta_book_style="style='background-color:green;color:white;'";
                            if($smarta_book=='실행중')$smarta_book_style="style='background-color:#c57900;color:white;'";
                            if($smarta_book=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($wehago_book=='에러')$wehago_book_style="style='background-color:#d10000;color:white;'";
                            if($wehago_book=='예약')$wehago_book_style="style='background-color:#007cc5;color:white;'";
                            if($wehago_book=='완료')$wehago_book_style="style='background-color:green;color:white;'";
                            if($wehago_book=='실행중')$wehago_book_style="style='background-color:#c57900;color:white;'";
                            if($wehago_book=='수동')$smarta_book_style="style='background-color:#f1e259;color:black;'";
                            
                            if($menual_down=='에러')$menual_down_style="style='background-color:#d10000;color:white;'";
                            if($menual_down=='예약')$menual_down_style="style='background-color:#007cc5;color:white;'";
                            if($menual_down=='완료')$menual_down_style="style='background-color:green;color:white;'";
                            if($menual_down=='실행중')$menual_down_style="style='background-color:#c57900;color:white;'";
                            if($menual_down=='수동')$menual_down_style="style='background-color:#f1e259;color:black;'";
                            ...
                            $proc == "" ? $output .= '<option value="" selected>전체</option>' : $output .='<option value="">전체</option>';
                            $proc == "E7227" ? $output .= '<option value="E7227" selected>사전안내</option>' : $output .='<option value="E7227">사전안내</option>';
                            $proc == "E7228" ? $output .= '<option value="E7228" selected>세액안내</option>' : $output .='<option value="E7228">세액안내</option>';
                            $proc == "E7229" ? $output .= '<option value="E7229" selected>사전추출</option>' : $output .='<option value="E7229">사전추출</option>';
                            $proc == "E7216" ? $output .= '<option value="E7216" selected>카톡등록</option>' : $output .='<option value="E7216">카톡등록</option>';
                            $proc == "E7217" ? $output .= '<option value="E7217" selected>입금확인</option>' : $output .='<option value="E7217">입금확인</option>';
                            $proc == "E7218" ? $output .= '<option value="E7218" selected>현영발행</option>' : $output .='<option value="E7218">현영발행</option>';
                            $proc == "E7219" ? $output .= '<option value="E7219" selected>수임동의</option>' : $output .='<option value="E7219">수임동의</option>';
                            $proc == "E7220" ? $output .= '<option value="E7220" selected>자료추출</option>' : $output .='<option value="E7220">자료추출</option>';
                            $proc == "E7221" ? $output .= '<option value="E7221" selected>회사등록</option>' : $output .='<option value="E7221">회사등록</option>';
                            $proc == "E7222" ? $output .= '<option value="E7222" selected>자동장부</option>' : $output .='<option value="E7222">자동장부</option>';
                            $proc == "E7223" ? $output .= '<option value="E7223" selected>신고작성</option>' : $output .='<option value="E7223">신고작성</option>';
                            $proc == "E7224" ? $output .= '<option value="E7224" selected>전자신고</option>' : $output .='<option value="E7224">전자신고</option>';
                            $proc == "E7225" ? $output .= '<option value="E7225" selected>완료안내</option>' : $output .='<option value="E7225">완료안내</option>';
                            $proc == "E7226" ? $output .= '<option value="E7226" selected>해지환불</option>' : $output .='<option value="E7226">해지환불</option>';
                            $output .='</select>
							</td>
                            ...
                            if($cash_report != ''){
                                ...
                                if($cash_report=="에러"||$cash_report=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_1\','.$row["ID"].');">요청</a></td>
                                    <td '.$cash_style.'><span data-tooltip-text="'.$ERR_MSG_cash_report.'" >'.$cash_report.'</span></td>';
                                }else{
                                    $output .= '</td><td '.$cash_style.'>'.$cash_report.'</span></td>';
                                }
                            }else{
                            ...
                            if($consign != ''){
                                ...
                                if($consign=="에러" || $consign=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_2\','.$row["ID"].');">요청</a></td>
                                    <td '.$consign_style.'><span data-tooltip-text="'.$ERR_MSG_consign.'" >'.$consign.'</span></td>';
                                }else{
                                    $output .= '</td><td '.$consign_style.'>'.$consign.'</span></td>';
                                }
                            }else{
                            ...
                            if($ht_print != ''){
                                $output .= '<td>';
                                if($ht_print=="에러" || $ht_print=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_6\','.$row["ID"].');">요청</a></td>
                                    <td '.$print_style.'><span data-tooltip-text="'.$ERR_MSG_ht_print.'" >'.$ht_print.'</span></td>';
                                }elseif($ht_print=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_6\','.$row["ID"].');">재요청</a></td>
                                    <td '.$print_style.'>'.$ht_print.'</td>';
                                }else{
                                    $output .= '</td><td '.$print_style.'>'.$ht_print.'</span></td>';
                                }
                            }else{
                            ...
                            */
                            ...
                            $output .= '<td>
                            <select style="height:35px;" id="user_ratio_'.$row["BIZ_ID"].'" name="bran_'.$row["BIZ_ID"].'" onchange="javascript:modify_option(this);">';
                            
                            $user_ratio == "" ? $output .= '<option value="100" selected>100%</option>' : $output .='<option value="100">100%</option>';
                            $user_ratio == "90" ? $output .= '<option value="90" selected>90%</option>' : $output .='<option value="90">90%</option>';
                            $user_ratio == "80" ? $output .= '<option value="80" selected>80%</option>' : $output .= '<option value="80">80%</option>';
                            $user_ratio == "70" ? $output .= '<option value="70" selected>70%</option>' : $output .='<option value="70">70%</option>';
                            $output .='</select>

                            </td>';
                            $output .= '<td><a target=_blank href="view_acc_cal.php?id='.$row["ID"].'">장부</a></td>';
                            if($smarta_book != ''){
                                $output .= '<td>';
                                if($smarta_book=="에러" || $smarta_book=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_A\','.$row["ID"].');">요청</a></td>
                                    <td '.$smarta_book_style.'><span data-tooltip-text="'.$ERR_MSG_smarta_book.'" >'.$smarta_book.'</span></td>';
                                }elseif($smarta_book=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_A\','.$row["ID"].');">재요청</a></td>
                                    <td '.$smarta_book_style.'>'.$smarta_book.'</td>';
                                }else{
                            ...
                            if($bran == "D1019"){
                                ...
                                $reguser == "1115" ? $output .= '<option value="1115" selected>한영순</option>' : $output .='<option value="1114">한영순</option>';
                                $reguser == "1165" ? $output .= '<option value="1165" selected>강지혜</option>' : $output .='<option value="1165">강지혜</option>';
                                $reguser == "1255" ? $output .= '<option value="1255" selected>임봉규</option>' : $output .='<option value="1255">임봉규</option>';
                                $reguser == "1254" ? $output .= '<option value="1254" selected>한유정</option>' : $output .='<option value="1254">한유정</option>';
                            }elseif ($bran=="D1021"){
...
<td>'.$row["REAL_REGUSER"].'</td>
                            <td>'.$douzone_svr.'</td>
                            <td>'.$douzone_code.'</td>';
                            
                            if($smartA != ''){
                                $output .= '<td>';
                                if($smartA=="에러" || $smartA=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_4\','.$row["ID"].');">요청</a></td>
                                    <td '.$smarta_style.'><span data-tooltip-text="'.$ERR_MSG_smartA.'" >'.$smartA.'</span></td>';
                                }elseif($smartA=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_4\','.$row["ID"].');">재요청</a></td>
                                    <td '.$smarta_style.'>'.$smartA.'</td>';
                                }else{
                            ...
                            if($ht_upload != ''){
                                ...
                                $output .= '<td>';
                                if($ht_upload=="에러" || $ht_upload=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_5\','.$row["ID"].');">요청</a></td>
                                    <td '.$upload_style.'><span data-tooltip-text="'.$ERR_MSG_ht_upload.'" >'.$ht_upload.'</span></td>';
                                }elseif($ht_upload=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_5\','.$row["ID"].');">재요청</a></td>
                                    <td '.$upload_style.'>'.$ht_upload.'</td>';
                                }else{
                            ...
                            if($menual_down != ''){
                                $output .= '<td>';
                                if($menual_down=="에러" || $menual_down=="수동"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_M\','.$row["ID"].');">요청</a></td>
                                    <td '.$menual_down_style.'><span data-tooltip-text="'.$ERR_MSG_menual_down.'" >'.$menual_down.'</span></td>';
                                }elseif($menual_down=="완료"){
                                    $output .= '<a href="javascript:cal_RPA(\'step_M\','.$row["ID"].');">재요청</a></td>
                                    <td '.$menual_down_style.'>'.$menual_down.'</td>';
                                }else{
                                    $output .= '</td><td '.$menual_down_style.'>'.$menual_down.'</span></td>';
                                }
                            }else{
                                $output .= '<td><a href="javascript:cal_RPA(\'step_M\','.$row["ID"].');">요청</a></td>
                                <td></td>';
                            }
                            ...
                            if($ht_upload == '완료'){
                                if($row["RP_SEND_KAKAO"] == 'Y')
                                    $output .='<td style="background-color:#ffdc40;text-align:center;padding:0;">발송<br>대기</td>';
                                        ...
                                        $output .='<td style="text-align:center;padding:0;"><a href="javascript:rp_kakao_send('.$row["BIZ_ID"].',\''.$row["CSTNAME"].'\')">발송</a></td>';
                            }else{
                                $output .='<td></td>';
                            }
                            if($cnt_kakao_send>0){
                                $output .='<td '.$style_kakao_send.'>'.$cnt_kakao_send.'건</td>';
                            }else{
                                $output .='<td></td>';
                            }
                            //$output .='</td>';
...
if($action == "select_list_simple2"){
	...
	LEFT OUTER JOIN TB300031 AS E ON A.CSTID=E.CSTID
    WHERE B.CST_TYPE = 'A1001' AND B.CST_TYPE_YEAR = '$SET_YEAR'
    AND B.INF_PATH='종소톡' ".$query_str1.$query_str2.$query_str3.$query_str_my;
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_PRO_SIMP2()
	BEGIN
	SELECT
    A.CSTID AS 'ID',
    CODE_TO_STR(B.REG_BRANCH) AS 'REG_BRANCH_PATH',
...
substring( REPLACE(A.MOBILE,'-',''),4) MOBILE_PATH,
	...
	";
        ...
        $hostname=$_SERVER["HTTP_HOST"];
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_PRO_SIMP2"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_PRO_SIMP2()";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
							...
							<select style="height:35px;" id="send_kakao_'.$row["BIZ_ID"].'" name="send_kakao_'.$row["BIZ_ID"].'">';
                                    ...
                                    $output .= '<option value="bizp_2022042114040413488193671" selected>빠른상담안내</option>';
                                    $output .= '<option value="bizp_2022042114014314053182657" >수수료 안내</option>';
                                    $output .= '<option value="bizp_2022042113590613488325669">세액  & 수수료 안내</option>';
                                    $output .= '<option value="bizp_2022042114030514053585658">예상세액 안내</option>';
                                    $output .= '<option value="bizp_2022042114073514053486659">부재중전화 안내</option>';
                                    $output .= '<option value="bizp_2022042114114213488987672">홈택스 안내문 조회방법</option>';
...
if($action == "select_list_kakao_send"){
    ...
    if($biz_id != ""){
        $query_str3 .= " AND B.BIZ_ID = ".$biz_id;
        $query = "SELECT COUNT(1) AS 'total'
            FROM TB100020 AS A
            LEFT OUTER JOIN TB100022 AS B ON A.CSTID=B.CSTID
            LEFT OUTER JOIN TB700010 AS C ON B.BIZ_ID=C.SEND_BIZ_ID
            WHERE C.SEND_FLAG='A1001' ".$query_str1.$query_str2.$query_str3;
    }else{
        $query = "SELECT COUNT(1) AS 'total'
            FROM TB100020 AS A
            LEFT OUTER JOIN TB100022 AS B ON A.CSTID=B.CSTID
            LEFT OUTER JOIN TB700010 AS C ON B.BIZ_ID=C.SEND_BIZ_ID
            WHERE C.SEND_TMP_STEP IN ('AUTO_COMPLATE') ".$query_str1.$query_str2;
    }
...
<col width="80px">
    ...
    SELECT A.CSTID AS 'CSTID' ,
            ...
            LEFT OUTER JOIN TB700010 AS C ON B.BIZ_ID=C.SEND_BIZ_ID
            #LEFT OUTER JOIN TB700020 AS D ON A.CSTID = D.CSTID
            WHERE C.SEND_FLAG='A1001'
            AND C.VISIBLE='Y'
            ".$query_str1.$query_str2.$query_str3.$query_desc.
            " LIMIT $limit_idx, $page_set
	;
...
<th>메모</th>
	...
	BEGIN
    SELECT A.CSTID, A.CSTNAME, A.MOBILE,FORMAT(B.DEP_FEE,0) AS DEP_FEE_ ,B.CASH_REPORT_APP_NUM, B.CASH_REPORT_REGDATE, B.MEMO
...
/*고객리뷰*/
if($action == "select_list_cst_review"){
    ...
    $query_str1 = "";
    $query_str2 = "";
    $query_desc = " ORDER BY B.REV_REGDATE DESC ";
    ...
    $query = "SELECT COUNT(1) AS 'total' FROM TB100020 AS A LEFT OUTER JOIN SS_REVIEW AS B
ON A.CSTNAME = B.REV_NAME  AND REPLACE(A.MOBILE,'-','')=REPLACE(B.REV_HP,'-','')
WHERE 1=1
AND IFNULL(B.REV_REGDATE,'') <> '' ".$query_str1.$query_str2;
    ...
    if(isset($_POST["action"]))
    {
        //users테이블 조회 프로시져를 만든다.
        $procedure = "
	CREATE PROCEDURE SELECT_LIST_CST_REVIEW()
	BEGIN
        SELECT 
        A.CSTNAME, A.MOBILE,
        B.REV_CONTENT,B.REV_SCORE, B.REV_REGDATE
        FROM TB100020 AS A LEFT OUTER JOIN SS_REVIEW AS B
        ON A.CSTNAME = B.REV_NAME  AND REPLACE(A.MOBILE,'-','')=REPLACE(B.REV_HP,'-','')
        WHERE 1=1
        AND IFNULL(B.REV_REGDATE,'') <> '' ".$query_str1.$query_str2.$query_desc.
    " LIMIT $limit_idx, $page_set
	;
	...
	";
        //기존에 프로시져가 존재한다면 지운다.
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_LIST_CST_REVIEW"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_LIST_CST_REVIEW()";
                ...
                if($result)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
                        ...
                        if($cst_type == "A1001"){
							...
							<td>'.$row["COMP_DATE_"].'</td>
							<td>'.$row["DEL_DATE_PAYMENT_"].'</span></td>
							<td><DIV id="memo_lbl_'.$row["ID"].'" style="width:200px;height:25px;padding-top:8px;" onclick="javascript:switch_comp(this)">'.mb_strimwidth($row['MEMO'],'0','40','...','utf-8').'</DIV><input style="width:200px; height:25px;display:none;" type="text" id="memo_ip_'.$row["ID"].'" value="'.$row['MEMO'].'" style="display:none;padding-top:10px;" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27)memo_submit(this);" ></input></td>
							</tr>';
                        }  // if($cst_type == "종합소득세") 끝
                    }
                }// 네번째 if문 끝
...
if($action == "select_cal_info"){
    ...
    LEFT OUTER JOIN TB300030 AS D ON A.CSTID = D.CSTID AND D.CST_TYPE_YEAR=DATE_FORMAT(now(), '%Y')-1
    LEFT OUTER JOIN TB100026 AS E ON B.BIZ_ID = E.BIZ_ID
    LEFT OUTER JOIN TB300031 AS F ON A.CSTID = F.CSTID AND F.CST_TYPE_YEAR=DATE_FORMAT(now(), '%Y')-1
...
, FORMAT(ADD_AMOUNT_PAID(P_ID),0) AS 'ADD_AMOUNT_PAID', EXT_YN, EXT_HUMAN
    FROM TB300020 WHERE CSTID = P_ID AND CST_TYPE_YEAR =DATE_FORMAT(now(), '%Y')-1
	;
...
if($action == "select_view_disc_ext1" && isset($_POST["id"])){
    ...
    $procedure = "
		CREATE PROCEDURE SELECT_VIEW_DISC_EXT1(IN user_id int(100))
		BEGIN
			SELECT CAL_CNT(user_id,'max','D') AS 'D_MAX_CNT',
CAL_CNT(user_id,'max','I') AS 'I_MAX_CNT',
CAL_CNT(user_id,'max','S') AS 'S_MAX_CNT',
CAL_CNT(user_id,'max','C') AS 'C_MAX_CNT',
G1_CAL_D(CAL_CNT(user_id,'max','D')) AS G1_CAL_D,
G1_CAL_D(CAL_CNT(user_id,'max','I')) AS G1_CAL_I,
G1_CAL_D(CAL_CNT(user_id,'max','S')) AS G1_CAL_S,
G1_CAL_D(CAL_CNT(user_id,'max','C')) AS G1_CAL_C,
CAL_CNT(user_id,'min','D') AS 'D_MIN_CNT',
CAL_CNT(user_id,'min','I') AS 'I_MIN_CNT',
CAL_CNT(user_id,'min','S') AS 'S_MIN_CNT',
CAL_CNT(user_id,'min','C') AS 'C_MIN_CNT',
G2_CAL_D(CAL_CNT(user_id,'min','D')) AS G2_CAL_D,
G2_CAL_D(CAL_CNT(user_id,'min','I')) AS G2_CAL_I,
G2_CAL_D(CAL_CNT(user_id,'min','S')) AS G2_CAL_S,
G2_CAL_D(CAL_CNT(user_id,'min','C')) AS G2_CAL_C,
CAL_CNT(user_id,'max','D') - CAL_CNT(user_id,'min','D') as 'D_DIV_CNT',
CAL_CNT(user_id,'max','I') - CAL_CNT(user_id,'min','I') as 'I_DIV_CNT',
CAL_CNT(user_id,'max','S') - CAL_CNT(user_id,'min','S') as 'S_DIV_CNT',
CAL_CNT(user_id,'max','C') - CAL_CNT(user_id,'min','C') as 'C_DIV_CNT',
G3_CAL_D(CAL_CNT(user_id,'max','D') - CAL_CNT(user_id,'min','D')) AS G3_CAL_D,
G3_CAL_D(CAL_CNT(user_id,'max','I') - CAL_CNT(user_id,'min','I')) AS G3_CAL_I,
G3_CAL_D(CAL_CNT(user_id,'max','S') - CAL_CNT(user_id,'min','S')) AS G3_CAL_S,
G3_CAL_D(CAL_CNT(user_id,'max','C') - CAL_CNT(user_id,'min','C')) AS G3_CAL_C

;
		END;
		";

    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_VIEW_DISC_EXT1"))
    {

        if(mysqli_query($connect, $procedure))
        {

            $query = "CALL SELECT_VIEW_DISC_EXT1(".$_POST["id"].")";
            $result = mysqli_query($connect, $query);
            $i = 0;
            while($row = mysqli_fetch_array($result))
            {

                $output['D_MAX_CNT'] = $row["D_MAX_CNT"];
                $output['I_MAX_CNT'] = $row["I_MAX_CNT"];
                $output['S_MAX_CNT'] = $row["S_MAX_CNT"];
                $output['C_MAX_CNT'] = $row["C_MAX_CNT"];
                $output['G1_CAL_D'] = $row["G1_CAL_D"];
                $output['G1_CAL_I'] = $row["G1_CAL_I"];
                $output['G1_CAL_S'] = $row["G1_CAL_S"];
                $output['G1_CAL_C'] = $row["G1_CAL_C"];
                $output['D_MIN_CNT'] = $row["D_MIN_CNT"];
                $output['I_MIN_CNT'] = $row["I_MIN_CNT"];
                $output['S_MIN_CNT'] = $row["S_MIN_CNT"];
                $output['C_MIN_CNT'] = $row["C_MIN_CNT"];
                $output['G2_CAL_D'] = $row["G2_CAL_D"];
                $output['G2_CAL_I'] = $row["G2_CAL_I"];
                $output['G2_CAL_S'] = $row["G2_CAL_S"];
                $output['G2_CAL_C'] = $row["G2_CAL_C"];
                $output['D_DIV_CNT'] = $row["D_DIV_CNT"];
                $output['I_DIV_CNT'] = $row["I_DIV_CNT"];
                $output['S_DIV_CNT'] = $row["S_DIV_CNT"];
                $output['C_DIV_CNT'] = $row["C_DIV_CNT"];
                $output['G3_CAL_D'] = $row["G3_CAL_D"];
                $output['G3_CAL_I'] = $row["G3_CAL_I"];
                $output['G3_CAL_S'] = $row["G3_CAL_S"];
                $output['G3_CAL_C'] = $row["G3_CAL_C"];
            }
...
if($action == "select_text_to_code"){
    ...
    $procedure = "
		CREATE PROCEDURE SELECT_TEXT_TO_CODE(IN P_USE_TEXT VARCHAR(50) )
		BEGIN
            SELECT (SELECT VALUE_ FROM TB750010_Y WHERE CODE_ = A.PARENT_CODE) AS 'VALUE_',  A.CODE_ AS 'CODE_' FROM dbsschina.TB750010_Y AS A WHERE A.VALUE_ = P_USE_TEXT;    
		END;
		";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TEXT_TO_CODE"))
    {
        
        if(mysqli_query($connect, $procedure))
        {
            
            $query = "CALL SELECT_TEXT_TO_CODE('".$use_text."')";
            $result = mysqli_query($connect, $query);
            $i = 0;
            while($row = mysqli_fetch_array($result))
            {
                $output['VALUE_'] = $row["VALUE_"];
                $output['CODE_'] = $row["CODE_"];
            }
...
if($action == "select_view_disc_ext2" && isset($_POST["id"])){
    ...
    $procedure = "
		CREATE PROCEDURE SELECT_VIEW_DISC_EXT2(IN P_USERID INT )
		BEGIN
            DECLARE CNT INT;
            			
            SELECT G1_NUM ,G2_NUM ,G3_NUM , G1_TYPE,G2_TYPE,G3_TYPE,
            RETURN_USER_LIST(G1_TYPE) AS 'GROUP_G1',
            RETURN_USER_LIST(G2_TYPE) AS 'GROUP_G2',
            RETURN_USER_LIST(G3_TYPE) AS 'GROUP_G3',
            REP_TYPE,
            SELECT_IMAGE_DISC(G1_TYPE) 'G1_IMAGE', 
            SELECT_IMAGE_DISC(G2_TYPE) 'G2_IMAGE', 
            SELECT_IMAGE_DISC(G3_TYPE) 'G3_IMAGE'
            FROM TB980090 WHERE USERID = P_USERID  
            ;
            
		END;
		";
    
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_VIEW_DISC_EXT2"))
    {
        
        if(mysqli_query($connect, $procedure))
        {
            
            $query = "CALL SELECT_VIEW_DISC_EXT2(".$userid.")";
            $result = mysqli_query($connect, $query);
            $i = 0;
            while($row = mysqli_fetch_array($result))
            {
                
                $output['TYPE_G1'] = $row["G1_TYPE"];
                $output['TYPE_G2'] = $row["G2_TYPE"];
                $output['TYPE_G3'] = $row["G3_TYPE"];
                $output['GROUP_G1'] = $row["GROUP_G1"];
                $output['GROUP_G2'] = $row["GROUP_G2"];
                $output['GROUP_G3'] = $row["GROUP_G3"];
                $output['REP_TYPE'] = $row["REP_TYPE"];
                $output['G1_IMAGE'] = $row["G1_IMAGE"];
                $output['G2_IMAGE'] = $row["G2_IMAGE"];
                $output['G3_IMAGE'] = $row["G3_IMAGE"];
...
// 부가세 고객 상세 뷰(기본) : 시작
if($action == "select_view_disc_test" && isset($_POST["id"])){
    ...
    $procedure = "
		CREATE PROCEDURE SELECT_VIEW_DISC_TEST(IN user_id int(100))
		BEGIN
			
            SELECT A.USERNAME, UPPER(CONCAT('Q',NUM,'_',FLAG)) AS 'QUE' ,VALUE as 'VALUE_' ,FLAG,NUM, DISC, 
            A.MOBILE, A.BRANCH,CODE_TO_STR(A.BRANCH) AS 'BRANCH_', A.REP_FLAG
            FROM dbsschina.TB980090 AS A
            LEFT OUTER JOIN TB980091 AS B
            ON A.USERID = B.USERID
            WHERE A.USERID = user_id
            order by B.NUM,B.FLAG;
		END;
		";
    //湲곗〈�쓽 �봽濡쒖떆��媛� 議댁옱�븳�떎硫� �궘�젣 �썑
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_VIEW_DISC_TEST"))
    {
        //�쐞�뿉�꽌 �꽑�뼵�븳 �봽濡쒖떆�� �꽑�뼵(1)
        if(mysqli_query($connect, $procedure))
        {
            //�봽濡쒖떆�� �샇異�(2)
            $query = "CALL SELECT_VIEW_DISC_TEST(".$_POST["id"].")";
            $result = mysqli_query($connect, $query);
            $i = 0;
            while($row = mysqli_fetch_array($result))
            {
                //�쐞�뿉�꽌 留뚮뱺 諛곗뿴�뿉 �꽔�뼱以��떎.
                $output['ID'] = $row["ID"];
                $output['USERNAME'] = $row["USERNAME"];
                $output['MOBILE'] = $row["MOBILE"];
                $output['BRANCH'] = $row["BRANCH"];
                $output['BRANCH_'] = $row["BRANCH_"];
                $output['REP_FLAG'] = $row["REP_FLAG"];
                $output['QUE'] = $row["QUE"];
...
if($action == "select_view_disc" && isset($_POST["id"])){
    ...
    $procedure = "
		...
		BEGIN
            ...
            SELECT_DEPT(INTERVIEW_REPORT) 'INTERVIEW_REPORT_', INTERVIEW_COMMENT, PROGRESS, RECEPTION, INCLUDE_SEV, FILE_VIEW_STR, FILE_REAL_STR
            FROM dbsschina.TB980090 AS A
            WHERE A.USERID = user_id
;
		END;
    ...
    if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_VIEW_DISC"))
    {
        ...
        if(mysqli_query($connect, $procedure))
        {
            ...
            while($row = mysqli_fetch_array($result))
            {
                ...
                $output['NUM'] = $row["NUM"];
                $output['DISC'] = $row["DISC"];
				$output['CAR_YEAR'] = $row["CAR_YEAR"];
				$output['CAR_MONTH'] = $row["CAR_MONTH"];
				$output['TOTAL_CAR_YEAR'] = $row["TOTAL_CAR_YEAR"];
				$output['TOTAL_CAR_MONTH'] = $row["TOTAL_CAR_MONTH"];
				$output['NEW_BEGIN'] = $row["NEW_BEGIN"];
				$output['TOTAL_NEW_BEGIN'] = $row["TOTAL_NEW_BEGIN"];
				$output['EVAL_USERID_'] = $row["EVAL_USERID_"];
				$output['EVAL_USERID'] = $row["EVAL_USERID"];
				$output['INTERVIEW_DATE'] = $row["INTERVIEW_DATE"];
				$output['INTERVIEW_DATE_'] = $row["INTERVIEW_DATE_"];
				$output['FINAL_EDU_'] = $row["FINAL_EDU_"];
				$output['FINAL_EDU'] = $row["FINAL_EDU"];
				$output['FINAL_SCHOOL'] = $row["FINAL_SCHOOL"];
				$output['IMPRESSION_'] = $row["IMPRESSION_"];
				$output['IMPRESSION'] = $row["IMPRESSION"];
				$output['DESIRE_'] = $row["DESIRE_"];
				$output['DESIRE'] = $row["DESIRE"];
				$output['KNOWLEDGE'] = $row["KNOWLEDGE"];
				$output['KNOWLEDGE_'] = $row["KNOWLEDGE_"];
				$output['ABILITY'] = $row["ABILITY"];
				$output['ABILITY_'] = $row["ABILITY_"];
				$output['PHYSICAL'] = $row["PHYSICAL"];
				$output['PHYSICAL_'] = $row["PHYSICAL_"];
				$output['TOTAL_EVAL'] = $row["TOTAL_EVAL"];
				$output['TOTAL_EVAL_'] = $row["TOTAL_EVAL_"];
				$output['INCLUDE_SEV'] = $row["INCLUDE_SEV"];
				$output['INTERVIEW_REPORT'] = $row["INTERVIEW_REPORT"];
				$output['INTERVIEW_REPORT_'] = $row["INTERVIEW_REPORT_"];
				$output['INTERVIEW_COMMENT'] = $row["INTERVIEW_COMMENT"];
				$output['PROGRESS'] = $row["PROGRESS"];
				$output['RECEPTION'] = $row["RECEPTION"];
				$output['BIRTH'] = $row["BIRTH_"];
				$output['AGE'] = $row["AGE"];
				$output['HOPE_MONEY'] = $row["HOPE_MONEY"];
...
if($action == "select_view_vat" && isset($_POST["id"])){
    ...
    $procedure = "
		...
		BEGIN
			...
			B.EST_FEE, B.DEP_FEE, B.DEP_TYPE,
    ...
    if (mysqli_query($connect, "DROP PROCEDURE IF EXISTS SELECT_MAIN_LINK")) {
        if (mysqli_query($connect, $procedure)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    if($row['LINK_NAME'] == "FAQ Note"){
                        $output .= '
            					<li>
                                    <span class="'.$row['FLAG'].'">'.$row['VALUE_'].'</span>
                                    <a href="javascript:pop_menu();" >'.$row['LINK_NAME'].'</a>
                                </li>
            					';
                    }else{
                        $output .= '
...
if($action == "select_trans" && isset($_POST["id"])){
    ...
    $procedure = "
		...
		BEGIN
			SELECT A.*, 
            FORMAT(A.TOTAL_TAX,0) 'TOTAL_TAX',
            B.USERNAME 'REGUSER_',
            C.CODE_ 'REG_BRANCH',
...
if($action == "select_trans_list"){
    ...
    if($t_option != "ALL" && $t_option != "" ){
        $WHERE_STR .= " AND A.OWNER_USER = '".$t_option."'  ";
    }else{
        $WHERE_STR .= " ";
    }
    ...
    if($_POST["s_option"] != ""){
        switch($s_option){
            case "cstname" :
                ...
                break;
            case "mobile" :
                $WHERE_STR .= " AND RETURN_STR(A.MOBILE) like RETURN_STR('%".$s_str."%') ";
                break;
            case "progress" :
                $WHERE_STR .= " AND D.VALUE_ like '%".$s_str."%' ";
                break;
            case "etc" :
            ...
            case "num" :
                ...
                break;
            case "deadline" :
                $WHERE_STR .= " AND DATE_FORMAT(A.DEADLINE BETWEEN, '%Y-%m-%d') BETWEEN '".$s_date1."' AND '".$s_date2."' ";
                break;
            case "rep_date" :
                $WHERE_STR .= " AND DATE_FORMAT(A.REP_DATE_ BETWEEN, '%Y-%m-%d') BETWEEN '".$s_date1."' AND '".$s_date2."' ";
                break;
            case "regdate" :
                $WHERE_STR .= " AND DATE_FORMAT(A.REGDATE, '%Y-%m-%d') BETWEEN '".$s_date1."' AND '".$s_date2."' ";
                break;
            case "path_info" :
                $WHERE_STR .= " AND K.VALUE_ like '%".$s_str."%' ";
                break;
            case "contents" :
                $WHERE_STR .= " AND REPLACE( regexp_replace(CONTENTS,'<[^>]+>','') , '&nbsp;','') like '%".$s_str."%' ";
                break;
            case "cons" :
                $WHERE_STR .= " AND CODE_TO_STR(VIP_CK) like '%".$s_str."%' ";
                break;
            default:
                $WHERE_STR ="";
        }
    }
    $query_str1 = $WHERE_STR;
    ...
    $query_cnt = "SELECT count(A.ID) as total FROM dbsschina.TB600010 AS A
            ...
            LEFT OUTER JOIN TB980010 AS I ON I.USERID = A.OWNER_USER
			LEFT OUTER JOIN TB750010 AS J ON J.CODE_ = A.PRIO_NUM
            LEFT OUTER JOIN TB750010 AS K ON K.CODE_ = A.PATH_INFO
    ...
    if($depid == "D1016" && $userid !=""){
        $query_str1 .= " AND A.REGUSER = '".$userid."'  ";
        $query_owner = "SELECT * FROM TB980010 WHERE USERID = '".$userid."' ORDER BY USERNAME;";
    }else{
        //$query_owner = "SELECT * FROM TB980010 WHERE DEPID IN ('D1013','D1016') ORDER BY USERNAME;";
        $query_owner = "SELECT * FROM TB980010 WHERE DEPID IN ('D1013') ORDER BY USERNAME;";
    }
	...
	BEGIN
		SELECT A.*, B.USERNAME 'REGUSER_',C.VALUE_ 'REG_BRANCH',D.VALUE_ 'PROGRESS_' ,
			E.VALUE_ 'TAX_FLAG',F.VALUE_ 'TRANS_TARGET',G.VALUE_ 'PAY_FLAG_',H.VALUE_ 'DELIVERY_FLAG',
            DATE_FORMAT(A.PAY_DATE, '%Y-%m-%d') 'PAY_DATE_',
			 DATE_FORMAT(A.TRANS_DATE, '%y-%m-%d') 'TRANS_DATE_',
            DATE_FORMAT(A.ACQ_DATE, '%y-%m-%d') 'ACQ_DATE_',
            DATE_FORMAT(A.DEADLINE, '%y-%m-%d') 'DEADLINE_',
        ...
        if(mysqli_query($connect,$procedure))
        {
            ...
            if(mysqli_num_rows($result) >0)
            {
                ...
                while($row = mysqli_fetch_array($result)){
                    ...
                    $prog == "E5009" ? $output .= '<option value="E5009" style="background-color:#146cc5;color:white" selected>검토요청</option>' : $output .='<option value="E5009" style="background-color:#146cc5;color:white">검토요청</option>';
                    //검토요청 : 빨간색
                    $prog == "E5010" ? $output .= '<option value="E5010" selected>검토완료</option>' : $output .='<option value="E5010">검토완료</option>';
                    $prog == "E5006" ? $output .= '<option value="E5006" style="background-color:green;color:white" selected>결재완료</option>' : $output .='<option value="E5006" style="background-color:green;color:white">결재완료</option>';
                    //결재완료 : 녹색
...
<td>
                            <select style="width:50px;height:35px;" id="vip_'.$row["ID"].'" name="vip_'.$row["ID"].'" onchange="javascript:modify_option_vip(this);">';
						
                            $vip == "" ? $output .= '<option value="" selected></option>' : $output .= '<option value=""></option>';
                            $vip == "H1001" ? $output .= '<option value="H1001" selected>상</option>' : $output .= '<option value="H1001">상</option>';
                            $vip == "H1002" ? $output .= '<option value="H1002" selected>중</option>' : $output .= '<option value="H1002">중</option>';
    						$vip == "H1003" ? $output .= '<option value="H1003" selected>하</option>' : $output .='<option value="H1003">하</option>';
					        $output .='</select></td>
                        ...
                        <td >
                        <select style="height:35px;" id="path_'.$row["ID"].'" name="path_'.$row["ID"].'" onchange="javascript:modify_option(this);">';
                            
					        $path == "" ? $output .= '<option value="" selected></option>' : $output .= '<option value=""></option>';
					        $path == "A3002" ? $output .= '<option value="A3002" selected>지점전화</option>' : $output .= '<option value="A3002">지점전화</option>';
					        $path == "A3112" ? $output .= '<option value="A3112" selected>광고전화</option>' : $output .= '<option value="A3112">광고전화</option>';
					        $path == "A3105" ? $output .= '<option value="A3105" selected>카톡</option>' : $output .='<option value="A3105">카톡</option>';
					        $path == "A3003" ? $output .= '<option value="A3003" selected>채널톡</option>' : $output .='<option value="A3003">채널톡</option>';
					        $path == "A3108" ? $output .= '<option value="A3108" selected>내방</option>' : $output .='<option value="A3108">내방</option>';
					        $path == "A3109" ? $output .= '<option value="A3109" selected>소개</option>' : $output .='<option value="A3109">소개</option>';
					        $path == "A3110" ? $output .= '<option value="A3110" selected>기존</option>' : $output .='<option value="A3110">기존</option>';
					        $path == "A3111" ? $output .= '<option value="A3111" selected>기타</option>' : $output .='<option value="A3111">기타</option>';
...
if($action == "login"){
	...
	BEGIN
		...
		WHERE ID=p_id 
        AND PW=CONCAT('*',UPPER(SHA1(UNHEX(SHA1(p_pw)))));
	END;
	";
...
if($action == "select_view" && isset($_POST["id"])){
    ...
    $procedure = "
		...
		BEGIN
			...
			FORMAT(E.WHI_TAX,0) AS WHI_TAX,
            FORMAT(E.EXI_TAX,0) AS EXI_TAX,
            FORMAT(E.NPIP,0) AS NPIP, 
            FORMAT(E.PERSON_SAVE,0) AS PERSON_SAVE ,
            FORMAT(E.SMALL_BIZ_DED,0) AS SMALL_BIZ_DED, 
            FORMAT(E.RET_SAVE,0) AS RET_SAVE , 
            FORMAT(E.PEN_SAVE,0) AS PEN_SAVE 
			FROM TB100020 AS A
...
if($action == "select_view_ext" && isset($_POST["id"])){
    ...
    if($_POST["flag"]=="TB310030"){
        
        $procedure = "
		CREATE PROCEDURE SELECT_TB310030(IN user_id int(100))
		BEGIN
			SELECT *, 
            FORMAT(TOTAL_PAYMENT,0) AS 'TOTAL_PAYMENT_',
FORMAT(INCOME_TAX,0) AS 'INCOME_TAX_',
FORMAT(LOCAL_INCOME_TAX,0) AS 'LOCAL_INCOME_TAX_',
FORMAT(TOTAL_INCOME_TAX,0) AS 'TOTAL_INCOME_TAX_'
             
			FROM TB310030 
            WHERE CSTID = user_id
            AND INCOME_ATTR_YEAR = $SET_YEAR;
		END;
		";
        
        if(mysqli_query($connect,"DROP PROCEDURE IF EXISTS SELECT_TB310030"))
        {
            //mysqli_query:DB에 쿼리 전송
            if(mysqli_query($connect,$procedure))
            {
                $query1 = "CALL SELECT_TB310030(".$_POST["id"].")";
                ...
                if(mysqli_num_rows($result) >0)
                {
                    ...
                    while($row = mysqli_fetch_array($result)){
						...
						<tr>
							...
							<td>'.$row["BIZ_NUM"].'</td>
							<td>'.$row["PAY_YEAR"].'</td>
							<td>'.$row["INCOME_ATTR_YEAR"].'</td>
							<td>'.$row["TOTAL_PAYMENT_"].' 원</td>
							<td>'.$row["INCOME_TAX_"].' 원</td>
							<td>'.$row["LOCAL_INCOME_TAX_"].' 원</td>
							<td>'.$row["TOTAL_INCOME_TAX_"].' 원</td>
						</tr>
    ...
    if($_POST["flag"]=="TB300020"){
		...
		BEGIN
			...
			(SELECT REPLACE( DUTY_TYPE ,'대상자','')FROM TB300010 
            WHERE CSTID = user_id) AS DUTY_TYPE_
			FROM TB300020 AS A LEFT OUTER JOIN TB100022 AS B ON A.CSTID = B.CSTID AND A.BIZ_ID = B.BIZ_ID
            WHERE A.CSTID = user_id
            AND B.CST_TYPE_YEAR = $SET_YEAR
            AND A.CST_TYPE_YEAR = $SET_YEAR;
		END;
		";
