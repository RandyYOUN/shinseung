...
$QUERY_STR = "&g_option=".$g_option."&b_option=".$_GET["b_option"]."&t_option=".$_GET["t_option"]."&s_str=".$_GET["s_str"]."&s_date1=".$s_date1."&s_date2=".$s_date2;
...
$QUERY_STR = "&s_option=".$_GET["s_option"]."&g_option=".$_GET["g_option"]."&t_option=".$_GET["t_option"]."&s_str=".$_GET["s_str"]."&sort=".$_GET["sort"]."&flag=".$_GET["flag"]."&c_option=".$_GET["c_option"]."&deadline=".$_GET["deadline"]."&rep_date=".$_GET["rep_date"]."&regdate=".$_GET["regdate"]."&s_date1=".$_GET["s_date1"]."&s_date2=".$_GET["s_date2"];
 
$search_message = "총 ". $total ."건의 결과가 검색되었습니다.";
?>
<div class="wrap">
		<div class="content">
			<div class="navi">
			...
			<div class="conwrap">
				...
				<div class="search">
					...
					if($userid=="1149"){
                        ?><button class="b_newadd"  name="kakao_option" id="kakao_option" style="background-color:#f0ed32;">알림톡설정</button>
                        <button class="b_newadd"  name="relation_option" id="relation_option" style="background-color:#f0ed32;">멘토설정</button>
                        <?php 
					}
					?>
					...
					<div class="selectbox w100p">
						...
						<?php 
						  ...
						  $output_owner .= '<select name="t_option" id="t_option">
<option value="">세무사 전체</option>';
					...
					<div class="selectbox w150p">
						...
						<select name="s_option" id ="s_option">
							...
							<option value="contents">보충조서</option>
							...
							<option value="path_info">유입채널</option>
							<option value="cons">컨설팅</option>
...
function modify_option_vip(obj){ //현황 저장
	var id_tmp = $(obj).attr("id");
	var id = id_tmp.replace("vip_","");
	var vip = document.getElementById(id_tmp).value;	
	var action = "upt_trans_opt_vip";
	var userid = "<?php echo $userid;?>";

	$.ajax({
		url:"action.php",
		method:"POST",
		data:{action:action,id:id,vip:vip, userid:userid},
		success:function(data){
			console.log(data);
			//alert(data);
			if(data=="true"){
				send_kakao_cons(id_tmp);
			}
		}
	})
...
function modify_option(obj){ //현황 저장
 	var id_tmp = $(obj).attr("id");
	if(id_tmp.indexOf("prog_") > -1)
		send_kakao_prog(id_tmp);
	
	var id = id_tmp.replace("prio_","");
	var id = id.replace("prog_","");
	var id = id.replace("pay_","");
	var id = id.replace("vip_","");
	var id = id.replace("path_","");
	...
	var vip_id = "vip_"+id;
	var vip = document.getElementById(vip_id).value;
	var path_id = "path_"+id;
	var path = document.getElementById(path_id).value;	
...
//	alert("id : "+ id + " = "+obj.value);
	...
	$.ajax({
		url:"action.php",
		method:"POST",
		data:{action:action,id:id,prio:prio,prog:prog, pay:pay, vip:vip, path:path},
		success:function(data){
			console.log(data);
			//alert("저장완료");
		}
...
//	if(userid =="1149"){
	//	window.location.href="view_trans.php?id="+id +"&t_option="+t_option+"&s_option="+s_option+"&g_option="+g_option+"&s_str="+escape(s_str)+"&s_date1="+s_date1+"&s_date2="+s_date2;
	//}else{
...
$(document).ready(function(){
	...
	if (s_option != "") {
		switch (s_option)
		{
			...
			case "etc" : 	
				...
				break;
			case "path_info" : 	
				$('#s_option').val('path_info').attr('selected','selected');
				break;
			case "deadline" : 	
			...
			case "contents" : 	
				...
				break;
			case "cons" : 	
				$('#s_option').val('cons').attr('selected','selected');
				break;
			//default : alert("error");
		}
	} 
	...
	if (t_option != "") {
		$('#t_option').val(t_option).attr('selected','selected');
	}
	...
	$('#btn_cancel').click(
		function(){
			window.location.href="?s_option=&g_option=&t_option=&s_str=&s_date1=&s_date2=";
		}	
	);
	...
	function checkit(){
		...
		var t_option = $('#t_option').val();
		var s_date1 = $('#s_date1').val();
		var s_date2 = $('#s_date2').val();
		var s_str = $('#s_str').val();

		if(s_option !="" ){
			window.location.href="?t_option="+t_option+"&s_option="+s_option+"&g_option="+g_option+"&s_str="+escape(s_str)+"&s_date1="+s_date1+"&s_date2="+s_date2;
		}else{
			alert("검색 조건을 설정해주세요");
			if(s_option ==""){
				$('#s_option').focus();
	...
	$('#btn_search').click(
		function() {
			...
			if(s_option !="" ){
				window.location.href="?s_option="+s_option+"&g_option="+g_option+"&t_option="+t_option+"&s_str="+escape(s_str)+"&s_date1="+s_date1+"&s_date2="+s_date2;	
			}else{
				alert("검색 조건을 설정해주세요");
				if(s_option ==""){
					$('#s_option').focus();
	...
	$('#kakao_option').click(
	function() {
		window.open("list_kakao_send_option.php","kakao_option","top=10, left=10, width=1000, height=500, status=no, menubar=no, toolbar=no, resizable=no");
	});


	$('#relation_option').click(
	function() {
		window.open("list_relation_setting.php","kakao_option","top=10, left=10, width=1000, height=500, status=no, menubar=no, toolbar=no, resizable=no");
	});
	...
	function() {
		...
		var userid="<?= $userid ?>";
		var depid="<?= $depid ?>";
		var go_url = "excel_down.php?s_option="+s_option+"&g_option="+g_option+"&t_option="+t_option+"&s_str="+s_str+"&depth="+depth+"&userid="+userid+"&depid="+depid+"&s_date1="+s_date1+"&s_date2="+s_date2;
	...
	function fetchUser()
	{
		...
		$.ajax({
			url:"select.php",
			method:"POST",
			data:{action:action,s_str:s_str,s_option:s_option,g_option:g_option,t_option:t_option,first:first, page:page,depth:depth,userid:userid,depid:depid,s_date1:s_date1,s_date2:s_date2},
			success:function(data){
				console.log(data);
				$('#result').html(data);
			}
	...
	$('#g_option').on('change',function(){
		var g_option = $('#g_option').val();
		window.location.href="?s_option="+s_option+"&g_option="+g_option+"&t_option="+t_option+"&s_str="+escape(s_str)+"&page="+page+"&s_date1="+s_date1+"&s_date2="+s_date2;
	});

	$('#t_option').on('change',function(){
		var t_option = $('#t_option').val();
		window.location.href="?s_option="+s_option+"&g_option="+g_option+"&t_option="+t_option+"&s_str="+escape(s_str)+"&page="+page+"&s_date1="+s_date1+"&s_date2="+s_date2;
	});
...
function send_kakao(str){
	...
	var id = str;
	var action = "action_tok_trans_vip";
...
/*컨설팅 카카오 발송*/
function send_kakao_cons(str){
	
	var cstid = str.replace("vip_","");
	var cons = document.getElementById(str).value;
	var action = "action_tok_trans_cons";
	var userid = <?php echo $userid;?>
	...
	if(cons!=""){
		...
		//insert page로 위에서 받은 데이터를 넣어준다.
			...
			method:"POST",
			data:{cstid:cstid,cons:cons,action:action,userid:userid},
			success:function(data){
				console.log(data);
...
function send_kakao_prog(str){
	
	var cstid = str.replace("prog_","");
	var prog = document.getElementById(str).value;
	var action = "action_tok_trans_prog";
	var userid = <?php echo $userid;?>

	if(prog=="E5006" || prog=="E5009"){

		$.ajax({
		//insert page로 위에서 받은 데이터를 넣어준다.
			url:"api/send_tok_v1.php", 
			method:"POST",
			data:{cstid:cstid,prog:prog,action:action,userid:userid},
			success:function(data){
				console.log(data);
...
function send_kakao_owner(cstid,str){
	
	//var cstid = str.replace("select_owner_","");
	var owner = str;
	var action = "action_tok_trans_owner";
	var userid = <?php echo $userid;?>
	...
	if(cstid !="" && str != ""){
		...
		//insert page로 위에서 받은 데이터를 넣어준다.
			url:"api/send_tok_v1.php", 
			method:"POST",
			data:{cstid:cstid,owner:owner,action:action,userid:userid},
			success:function(data){
				console.log(data);
