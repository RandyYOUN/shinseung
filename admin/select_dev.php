			...
			if(mysqli_num_rows($result) >0)
			{
				...
				while($row = mysqli_fetch_array($result)){
					...
					<tr '.$style.' >
						...
						<td style="padding:10px;"><a href="view_trans.php?id='.$row["ID"].'" title="'.$row["CSTNAME"].'"> '.mb_strimwidth($row['CSTNAME'],'0','12','...','utf-8').'</a></td>
						<td>'.$row["MOBILE"].'</td>
						<td>'.$row["TAX_FLAG"].'</td>
						...
						<td><DIV id="memo_lbl_'.$row["ID"].'" style="width:280px;height:25px;padding-top:8px;" onclick="javascript:switch_comp(this)">'.$row["ETC"].'</DIV><input style="width:280px; height:25px;display:none;" type="text" id="memo_ip_'.$row["ID"].'" onKeypress="javascript:if(event.keyCode==13 || event.keyCode==27) {memo_submit(this);}" value="'.$row["ETC"].'" style="display:none;padding-top:10px;"></input></td>
...
if($action == "select_memberlist"){
	...
	BEGIN
		...
		LEFT OUTER JOIN TB750010 AS C ON A.POSITION_ID = C.CODE_
		WHERE 1=1 ".$query_str."
		AND DEPID NOT IN ('D1000','D1999','D1016')
		AND USERID NOT IN(1167,1168,1169,1171,1175)
		ORDER BY DEPID, INNER_PHONE;
	END;
	";
