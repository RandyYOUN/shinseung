...
?>		
		...
		<div class="content" style="width: 1810px;">
			<div class="navi">
			</div>
...
if($s_str !=""){
    switch($g_option){
        case "NAME" :
            ...
            break;
        case "MOBILE" :
            $query_str1 .= " AND REPLACE(MOBILE,'-','') like '%".$s_str."%' ";
            break;
        default:
            $query_str1 .="";
    }
...
?>
	...
	<input type=hidden id="page_flag" value="알림톡로그 상세페이지">
</body>
...
$(document).ready(function(){
	...
	function fetchUser()
	{
		...
		var req = new Request();
		var action = "select_list_kakao_send";
		var b_option = $('#b_option').val();
		var g_option = $('#g_option').val();
		var s_str = $('#s_str').val();
		var page = "";
		page = req.getParameter("page");
		biz_id = req.getParameter("biz_id");
		...
		$.ajax({
			url:"select.php",
			method:"POST",
			data:{action:action,s_str:s_str,b_option:b_option,g_option:g_option, page:page, biz_id:biz_id},
	...
	$('#excel').click(
		function() {
			...
			var userid="<?= $userid ?>";
			var depid="<?= $depid ?>";
			var go_url = "excel_down_kakaolog.php?s_option="+s_option+"&g_option="+g_option+"&s_str="+s_str+"&depth="+depth+"&userid="+userid+"&depid="+depid;
...
function send_sms(cstid){

	var action ="send_sms_self";
	var userid = "<?php echo $userid?>";
	$.ajax({
		url:"action.php",
		method:"POST",
		data:{action:action,cstid:cstid,userid:userid},
		success:function(data){
			alert(data);
			//alert(data.result);
			//$('#result').html(data);
