					...
					<table>
						<tbody>
							<tr>
								...
								<th>PW</th>
								<td>
									<input type="box" class="w50" name="member_pw" id="member_pw" disabled>
								</td>
							</tr>
							<tr>
								<th>직원명</th>
							...
							<tr>
								<th>내선번호</th>
								<td>
									<input type="box" class="w50" name="inner_phone" id="inner_phone">
								</td>
								<th>대표번호</th>
								<td>
									<input type="box" class="w50" name="outer_phone" id="outer_phone">
								</td>
							</tr>
							<tr>
								<th>핸드폰</th>
							...
							<TR>
								...
								<TD colspan=3>
									<div class="selectbox s50">
										...
										<option value="">선택</option>
										<option value="D2001">회장</option>
										<option value="D2002">대표이사</option>
										<option value="D2003">본부장</option>
										<option value="D2005">지점장</option>
										<option value="D2101">고문</option>
										<option value="D2106">전무</option>
										<option value="D2109">상무</option>
										<option value="D2105">이사</option>
										<option value="D2103">부장</option>
										<option value="D2108">차장</option>
										<option value="D2102">대리</option>
										<option value="D2104">사원</option>
									</select>
								</TD>
							</TR>
						</tbody>
...
function fetchUser()
{
	...
	$('#action').click(function(){
		...
		var action = "reg_member_admin";

		//성과 이름이 올바르게 입력이 되면
		if(member_id !='' && member_name != '' && depid != ''){

			$.ajax({
			//insert page로 위에서 받은 데이터를 넣어준다.
				url:"action.php", 
				method:"POST",
				data:{member_id:member_id,member_name:member_name,depid:depid,action:action, position_id:position_id, inner_phone:inner_phone,outer_phone:outer_phone,mobile:mobile },
				success:function(data){
					alert(data);
