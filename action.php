...
{ 
	...
	{ 
		...
		BEGIN
		INSERT INTO NEW_HP(NEW_HP,Q_TYPE,REGDATE,GUBUN) VALUES(NEW_HP,Q_TYPE,NOW(),'세무톡');
		END
		";
		...
		{ //위에서 만든 프로시저 실행
			if(mysqli_query($connect,$procedure))
			{
				...
				}else if($Q_TYPE == '양도상속증여'){
					$_param['phone'] = '01092587740'; // 김세현
				}else if($Q_TYPE == '제휴문의'){
					$_param['phone'] = '01030925352'; // 노준석
				}
				//$_param['phone'] = $NEW_HP;       // [필수] 수신번호 - 여러명일 경우 |로 구분 '010********|010********|010********'

				$_param['msg'] ='세무톡 사이트에서 '. $NEW_HP.' 번호로 ['.$Q_TYPE. '] 전화상담 문의가 왔습니다. 응대부탁드립니다.';
				$_param['subject'] = '응대요청';
